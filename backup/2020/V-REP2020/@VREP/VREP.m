classdef VREP < handle
    ...
    methods
        ...
        % Laser Scanner
        [Map] = vGetLaserData(vrep,p3dx,Index);
        
        % Data Request
        % Get Data from Pioneer
        vGetSensorData(vrep,p3dx,Index); % Xc - Center Position, X - Control Position, U - Velocity 
        % Get Data from Objects
        [Xc,O] = vGetObjPosition(vrep,String); % Xc - Center Position, O - Orientation
        
        % Send Command
        vSendControlSignals(vrep,p3dx,Index); % Send Control Signals to V-REP
