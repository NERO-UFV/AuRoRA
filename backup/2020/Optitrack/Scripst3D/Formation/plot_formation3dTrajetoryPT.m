...
% data = load('FL3d_TrajElipseExp20181011T122543.txt');
data = load('FL3d_TrajectoryUtimateExpNewControllers20200110T112433.txt');
...
% % Desired positions
pd1 = plot3(Qd(1,:),Qd(2,:),PXd(3,:)+h,'r--','LineWidth',1);hold on % pioneer
pd2 = plot3(x2,y2,z2,'b--','LineWidth',1);     % drone
...
%     
    ...
    % Percourse made
  pm1 =  plot3(PX(1,1:k),PX(2,1:k),PX(3,1:k)+h,'r-','LineWidth',sizeReal); hold on;
  pm2 =  plot3(AX(1,1:k),AX(2,1:k),AX(3,1:k),'b-','LineWidth',sizeReal);
...
% lg1 = legend([ps1 pd1 pd2],{'Posi\c{c}\~{a}o inicial','Trajet\''{o}ria desejada Pioneer 3-DX','Trajet\''{o}ria desejada AR.Drone2.0'});
...
pdd1 = plot3(Qd(1,:),Qd(2,:),PXd(3,:)+h,'r--','LineWidth',sizeDesired);hold on % pioneer
...
% Ponto final do trecho
pend1 = plot3(PX(1,lap),PX(2,lap),PX(3,lap)+h,'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
