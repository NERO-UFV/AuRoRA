%% Rotina para plotar gráficos de Formacaolinha3D - 
%  Drone - Drone
clear
close all
clc

%% Rotina para buscar pasta raiz
PastaAtual = pwd;
PastaRaiz = 'AuRoRA 2018';
cd(PastaAtual(1:(strfind(PastaAtual,PastaRaiz)+numel(PastaRaiz)-1)))
addpath(genpath(pwd))

%% Declara os robos
AR1  = ArDrone;
AR2  = ArDrone;
LF = LineFormation3D;

%% Carrega dados
data = load('FL3d_2DronesSimTraj20181201T165424.txt');

%% Atribuição de variáveis
%Tempo da simulação
time  = data(:,end);             % tempo (s)

% Pioneer data
A1.Xd   = data(:,(1:12))';       % desired pose
A1.X    = data(:,12+(1:12))';    % real pose
A1.Ud   = data(:,24+(1:4))';     % control signal
A1.U    = data(:,28+(1:4))';     % velocidades do robô
A1.Xtil = A1.Xd - A1.X;          % erro de postura

% Drone data
A2.Xd   = data(:,32+(1:12))';
A2.X    = data(:,44+(1:12))';
A2.Ud   = data(:,56+(1:4))';
A2.U    = data(:,60+(1:4))';
A2.Xtil = A2.Xd - A2.X;

% Formation data
Qd   = data(:,64+(1:6))';   % desired formation
Q    = data(:,70+(1:6))';   % real formation

% Alpha quadrant adjust
% a = 1;
% for ii = 1:length(Q)-1
%     angle = Q(5,ii+1) - Q(5,ii);
%     if abs(angle)<6.5  && abs(angle) > 6  % detect breaks
%         ind(a) = ii;   % save break vector indice
%         a = a+1;
%     end
% end      % detect breaks
% 
% for f = 1:2:length(ind)-1 
%     for jj = ind(f):ind(f+1)-1
%         if Q(5,jj+1)>0      % -pi --> +pi
%             Q(5,jj+1) = Q(5,jj+1) - 2*pi;
%         elseif Q(5,jj+1)<0  % +pi --> -pi
%             Q(5,jj+1) = Q(5,jj+1) + 2*pi;
%         end
%     end
% end  % do the thing

Qtil = Qd - Q;              % formation error

% Robots desired positions
for kk = 1:length(Qd)
    xf   = Qd(1,kk);  yf    = Qd(2,kk);   zf = Qd(3,kk);
    rhof = Qd(4,kk);  alfaf = Qd(5,kk);   betaf = Qd(6,kk);
    
    x2(kk) = xf + rhof*cos(alfaf)*cos(betaf);
    y2(kk) = yf + rhof*sin(alfaf)*cos(betaf);
    z2(kk) = zf + rhof*sin(betaf);
end

% Quadrant correction 
for k=1:length(Qd)
    if abs(Qd(5,k)) > pi
        if Qd(5,k) > 0
            Qd(5,k) = Qd(5,k) - 2*pi;
        else
            Qd(5,k) = Qd(5,k) + 2*pi;
        end
    end
    if abs(Q(5,k)) > pi
        if Q(5,k) > 0
            Q(5,k) = Q(5,k) - 2*pi;
        else
            Q(5,k) = Q(5,k) + 2*pi;
        end
    end
     if abs(Qtil(5,k)) > pi
        if Qtil(5,k) > 0
            Qtil(5,k) = Qtil(5,k) - 2*pi;
        else
            Qtil(5,k) = Qtil(5,k) + 2*pi;
        end
    end
end




%% PLOTA RESULTADOS
% Tamanho das letras, gráficos e símbolos
sizeDesired = 2.5;      % largura das linhas de valores desejados
sizeReal    = 1;      % largura da linha de valor real
sizeLabel   = 14;     % tamanho da fonte dos labels
sizeSymbol  = [8 2];  % [tamanho dos símbolos   espessura da linha]
sizeLegenda = 15;     % tamanho das legendas
location    = 'SouthEast'; %'NorthEast';
% Posição dos robôs
% Parameters
step   = 200;   % model plot step

figure;
axis equal
axis ([-5 5 -5 5 0 7])
set(gca,'Box','on')
hold on, grid on;

% Initial positions
ps1 = plot3(A1.X(1,1),A1.X(2,1),A1.X(3,1),'k^','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
ps2 = plot3(A2.X(1,1),A2.X(2,1),A2.X(3,1),'k^','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));

% % % Final positions
% ps1 = plot3(A1.X(1,end),A1.X(2,end),A1.X(3,end),'ksq','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% ps2 = plot3(A2.X(1,end),A2.X(2,end),A2.X(3,end),'ksq','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));

% 
% % Percourse made
% p1 = plot3(PX(1,:),PX(2,:),PX(3,:)+h,'r-','LineWidth',0.5);
% p2 = plot3(AX(1,:),AX(2,:),AX(3,:),'b-','LineWidth',0.5);
xlabel('$x$ [m]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$y$ [m]','FontSize',sizeLabel,'Interpreter','latex');
zlabel('$z$ [m]','FontSize',sizeLabel,'interpreter','Latex');

% % Desired positions
pd1 = plot3(Qd(1,:),Qd(2,:),Qd(3,:),'k--','LineWidth',sizeDesired);hold on % drone 1
pd2 = plot3(x2,y2,z2,'k--','LineWidth',sizeDesired);     % drone 2


% plot robots and formation lines
for k = 1:step:length(time)
    % ArDrone position
    AR1.pPos.X = A1.X(:,k);
    AR2.pPos.X = A2.X(:,k);

    % Plota drones
     AR1.mCADplot;
     AR2.mCADplot;
    
    drawnow
%     view(60,15)
    
    % plot formation line
    x = [A1.X(1,k)   A2.X(1,k)];
    y = [A1.X(2,k)   A2.X(2,k)];
    z = [A1.X(3,k)   A2.X(3,k)];
    
    pl = line(x,y,z);
    pl.Color = 'g';
    pl.LineStyle = '-';
    pl.LineWidth = 1;
%     
    
    %     hold on
    %     % plot desired formation line
    %     xd = [Xd(1,k)    AXd(1,k)];
    %     yd = [Xd(2,k)    AXd(2,k)];
    %     zd = [Xd(3,k)+h  AXd(3,k)];
    %
    %     pld = line(xd,yd,zd);
    %     pld.Color = 'm';
    %     pld.LineStyle = '--';
    
    % Percourse made
  pm1 =  plot3(A1.X(1,1:k),A1.X(2,1:k),A1.X(3,1:k),'r-','LineWidth',sizeReal); hold on;
  pm2 =  plot3(A2.X(1,1:k),A2.X(2,1:k),A2.X(3,1:k),'b-','LineWidth',sizeReal);
    
    
%     pause(1/30)
end
% %
% % lg1 = legend([pl ps1 pd1 pd2],{'Linha da forma\c{c}\~{a}o','Posi\c{c}\~{a}o inicial','Trajet\''{o}ria desejada Pioneer 3-DX','Trajet\''{o}ria desejada AR.Drone2.0'});
lg1 = legend([ps1 pl pd1 pm1 pm2],{'Posi\c{c}\~{a}o inicial','Forma\c{c}\~{a}o','Trajet\''{o}ria desejada','Trajet\''{o}ria real rob\^o 1','Trajet\''{o}ria real rob\^o 2'});
% % lg1 = legend([ps1 pd1 pd2 pm1 pm2],{'Posi\c{c}\~{a}o inicial','Trajet\''{o}ria desejada Pioneer 3-DX','Trajet\''{o}ria desejada AR.Drone 2.0'...
% %     'Trajet\''{o}ria realizada Pioneer 3-DX','Trajet\''{o}ria realizada AR.Drone 2.0'});
% % 
% lg1 = legend([ps1 pd1],{'Start position','Desired Positions'});
% % % lg1 = legend(pl,{'Formation line'});
% % %
lg1.FontSize = sizeLegenda;
lg1.Location = 'SouthEast';
set(lg1,'Interpreter','latex');
% % 
% % ##########################################################################
% %% Separated windows
% % ##########################################################################
% % 
% figure;
% axis equal
% axis ([-2 2 -2 2 0 2])
% set(gca,'Box','on')
% 
% hold on, grid on;
% 
% % Initial positions
% pstart1 = plot3(X(1,1),X(2,1),X(3,1)+h,'k^','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% pstart2 = plot3(AX(1,1),AX(2,1),AX(3,1),'k^','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% 
% % % Final positions
% pend1 = plot3(X(1,end),X(2,end),X(3,end)+h,'ksq','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% pend2 = plot3(AX(1,end),AX(2,end),AX(3,end),'ksq','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% 
% 
% % % Percourse made
% pp1 = plot3(X(1,:),X(2,:),X(3,:)+h,'r-','LineWidth',sizeReal);
% pp2 = plot3(AX(1,:),AX(2,:),AX(3,:),'b-','LineWidth',sizeReal);
% % title('Posição dos Robôs','fontSize',lt);
% xlabel('$x$ [m]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('$y$ [m]','FontSize',sizeLabel,'Interpreter','latex');
% zlabel('$z$ [m]','FontSize',sizeLabel,'interpreter','Latex');
% 
% % % Desired positions
% % pdd1 = plot3(Qd(1,:),Qd(2,:),PXd(3,:)+h,'k--','LineWidth',sizeDesired); % pioneer
% % pdd2 = plot3(x2,y2,z2,'k--','LineWidth',sizeDesired);     % drone
% pdd1 = plot3(Qd(1,:),Qd(2,:),Xd(3,:)+h,'r--','LineWidth',sizeDesired);hold on % pioneer
% pdd2 = plot3(x2,y2,z2,'b--','LineWidth',sizeDesired);     % drone
% 
% % %Formation line
% % vec = [1 length(PX)];
% % for k = 1:length(vec)
% %     x = [PX(1,vec(k))    AX(1,vec(k))];
% %     y = [PX(2,vec(k))    AX(2,vec(k))];
% %     z = [PX(3,vec(k))+h   AX(3,vec(k))];
% %     
% %     ppl = line(x,y,z);
% %     ppl.Color = 'g';
% %     ppl.LineStyle = '-';
% %     ppl.LineWidth = 1;
% % end
% % 
% 
% % leg1 = legend([ppl pstart1 pend1 pdd1 pdd2],{'Linha da forma\c{c}\~{a}o','Posi\c{c}\~{a}o inicial','Posi\c{c}\~{a}o final','Trajet\''{o}ria desejada Pioneer 3-DX','Trajet\''{o}ria desejada AR.Drone 2.0'});
% % leg1 = legend([pstart1 pend1 pdd1 pdd2],{'Posi\c{c}\~{a}o inicial','Posi\c{c}\~{a}o final','Trajet\''{o}ria desejada Pioneer 3-DX','Trajet\''{o}ria desejada AR.Drone 2.0'});
% leg1 = legend([pstart1 pend1  pdd1 pdd2 pp1 pp2],{'Posi\c{c}\~{a}o inicial',...
%     'Posi\c{c}\~{a}o  final','Trajet\''{o}ria desejada Pioneer 3-DX', ...
%     'Trajet\''{o}ria desejada AR.Drone 2.0','Trajet\''{o}ria realizada Pioneer 3-DX',...
%     'Trajet\''{o}ria realizada AR.Drone 2.0'});
% 
% leg1.FontSize = sizeLegenda;
% leg1.Location = location;
% set(leg1,'Interpreter','latex');
% % Superior View *********************************************************
% figure;
% axis equal
% axis ([-2 2 -2 2])
% set(gca,'Box','on')
% 
% hold on, grid on;
% 
% % Initial positions
% pstart1 = plot(X(1,1),X(2,1),'k^','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% pstart2 = plot(AX(1,1),AX(2,1),'k^','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% 
% % % Final positions
% pend1 = plot(X(1,end),X(2,end),'ksq','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% pend2 = plot(AX(1,end),AX(2,end),'ksq','MarkerSize',sizeSymbol(1),'LineWidth',sizeSymbol(2));
% 
% 
% % % Percourse made
% preal1 = plot(X(1,:),X(2,:),'r-','LineWidth',sizeReal);
% preal2 = plot(AX(1,:),AX(2,:),'b-','LineWidth',sizeReal);
% % title('Posição dos Robôs','fontSize',lt);
% xlabel('$x$ [m]','interpreter','Latex'),ylabel('$y$ [m]','Interpreter','latex');
% 
% % % Desired positions
% pdes1 = plot(Qd(1,:),Qd(2,:),'r--','LineWidth',sizeDesired); % pioneer
% pdes2 = plot(x2,y2,'b--','LineWidth',sizeDesired);     % drone
% 
% 
% leg1 = legend([pstart1 pend1 pdes1 pdes2 preal1 preal2],{'Posi\c{c}\~{a}o inicial', ...
%     'Posi\c{c}\~{a}o final','Trajet\''{o}ria desejada Pioneer 3-DX', ...
%     'Trajet\''{o}ria desejada AR.Drone 2.0', ...
%     'Trajet\''{o}ria real Pioneer3-DX','Trajet\''{o}ria real AR.Drone 2.0'});
% 
% leg1.FontSize = sizeLegenda;
% leg1.Location = location;
% set(leg1,'Interpreter','latex');
% 
% % ######################################################################
% %% Velocidades 
% % Pioneer ****************************************************************
% figure;
% plot(time,Ud(1,:),'b--','LineWidth',sizeDesired), hold on;
% plot(time,PU(1,:),'r','LineWidth',sizeReal);
% legend({'$u_{d}$','$u_{r}$'},'FontSize',sizeLegenda,'Location',location,'Interpreter','latex');
% grid on,xlim([0 time(end)]);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Velocidade linear [m/s]','FontSize',sizeLabel,'interpreter','Latex');
% 
% figure;
% plot(time,Ud(2,:),'b--','LineWidth',sizeDesired), hold on;
% plot(time,PU(2,:),'r','LineWidth',sizeReal)
% legend({'$\omega_{d}$','$\omega_{r}$'},'FontSize',sizeLegenda,'Location',location,'Interpreter','latex');
% grid on,xlim([0 time(end)]);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Velocidade angular [rad/s]','FontSize',sizeLabel,'interpreter','Latex');
% 
% % ArDrone ****************************************************************
% % phi
% figure;
% plot(time,AUd(1,:),'b--','LineWidth',sizeDesired);hold on;
% plot(time,AU(1,:),'r','LineWidth',sizeReal);
% legend({'$\phi_{d}$','$\phi_{r}$'},'FontSize',sizeLegenda,'Location',location,'Interpreter','latex');
% grid on,xlim([0 time(end)]);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Rolagem [rad]','FontSize',sizeLabel,'interpreter','Latex');
% 
% % theta
% figure;
% plot(time,AUd(2,:),'b--','LineWidth',sizeDesired);hold on;
% plot(time,AU(2,:),'r','LineWidth',sizeReal);
% legend({'$\theta_{d}$','$\theta_{r}$'},'FontSize',sizeLegenda,'Location',location,'Interpreter','latex');
% grid on,xlim([0 time(end)]);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Arfagem [rad]','FontSize',sizeLabel,'interpreter','Latex');
% 
% % dZ
% figure;
% plot(time,AUd(3,:),'b--','LineWidth',sizeDesired);hold on;
% plot(time,AU(3,:),'r','LineWidth',sizeReal);
% legend({'$\dot{z}_{d}$','$\dot{z}_{r}$'},'FontSize',sizeLegenda,'Location',location,'Interpreter','latex');
% grid on,xlim([0 time(end)]),ylim([-1 1]);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Varia\c{c}\~{a}o de altitude [m/s]','FontSize',sizeLabel,'interpreter','Latex');
% 
% % dPsi
% figure;
% plot(time,AUd(4,:),'b--','LineWidth',sizeDesired);hold on;
% plot(time,-AU(4,:),'r','LineWidth',sizeReal);
% legend({'$\dot{\psi}_{d}$','$\dot{\psi}_{r}$'},'FontSize',sizeLegenda,'Location',location,'Interpreter','latex');
% grid on,xlim([0 time(end)]);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Taxa de guinada [rad/s]','FontSize',sizeLabel,'interpreter','Latex');
% 
% 
% % Erro da formação ****************************************************
% 
% Position
figure;
plot(time,Qtil(1,:),'b--','LineWidth',sizeReal),hold on;
plot(time,Qtil(2,:),'r-','LineWidth',sizeReal);
plot(time,Qtil(3,:),'k--','LineWidth',sizeReal+1);
plot(time,Qtil(4,:),'g-.','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('Erro [m]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

lgg3 = legend('$x_f$','$y_f$','$z_f$','$\rho_f$');
lgg3.FontSize = 14;
lgg3.Location = location;
set(lgg3,'Interpreter','latex');
grid on;


% Angles
figure;
plot(time,rad2deg(Qtil(5,:)),'b--','LineWidth',sizeReal),hold on;
plot(time,rad2deg(Qtil(6,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('Erro [graus]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

lgg4 = legend('$\alpha_f$','$\beta_f$');
lgg4.FontSize = 14;
lgg4.Location = location;
set(lgg4,'Interpreter','latex');
grid on

% % #########################################################################
%% Plota variáveis da formação
% xf
figure;
subplot(321)
plot(time,(Qd(1,:)),'b--','LineWidth',sizeDesired),hold on;
plot(time,(Q(1,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$x_f$ [m]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

% lgg1 = legend('$x_{fd}$','$x_{fr}$');
% lgg1.FontSize = 14;
% lgg1.Location = location;
% set(lgg1,'Interpreter','latex');
grid on

% yf
% figure;
subplot(323)
plot(time,(Qd(2,:)),'b--','LineWidth',sizeDesired),hold on;
plot(time,(Q(2,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$y_f$ [m]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

% lgg2 = legend('$y_{fd}$','$y_{fr}$');
% lgg2.FontSize = 14;
% lgg2.Location = location;
% set(lgg2,'Interpreter','latex');
grid on

% zf
% figure;
subplot(325)
plot(time,(Qd(3,:)),'b--','LineWidth',sizeDesired),hold on;
plot(time,(Q(3,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$z_f$ [m]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

% lgg2 = legend('$y_{fd}$','$y_{fr}$');
% lgg2.FontSize = 14;
% lgg2.Location = location;
% set(lgg2,'Interpreter','latex');
grid on

% rhof
% figure;
subplot(322)
plot(time,(Qd(4,:)),'b--','LineWidth',sizeDesired),hold on;
plot(time,(Q(4,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$\rho_f$ [m]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

% lgg4 = legend('$\rho_{fd}$','$\rho_{fr}$');
% lgg4.FontSize = 14;
% lgg4.Location = location;
% set(lgg4,'Interpreter','latex');
grid on

% alphaf
% figure;
subplot(324)
plot(time,rad2deg(Qd(5,:)),'b--','LineWidth',sizeDesired),hold on;
plot(time,rad2deg(Q(5,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$\alpha_f$ [graus]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('\^Angulo [graus]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

% lgg5 = legend('$\alpha_{fd}$','$\alpha_{fr}$');
% lgg5.FontSize = 14;
% lgg5.Location = location;
% set(lgg5,'Interpreter','latex');
grid on

% betaf
% figure;
subplot(326)
plot(time,rad2deg(Qd(6,:)),'b--','LineWidth',sizeDesired),hold on;
plot(time,rad2deg(Q(6,:)),'r-','LineWidth',sizeReal);
xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
ylabel('$\beta$ [graus]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('\^Angulo [graus]','FontSize',sizeLabel,'interpreter','Latex');
xlim([0 time(end)]);

lgg6 = legend('Desejado','Realizado');
% lgg6 = legend('$\beta_{fd}$','$\beta_{fr}$');
lgg6.FontSize = 14;
lgg6.Location = location;
set(lgg6,'Interpreter','latex');
grid on

% % #########################################################################
% %% Variáveis dos robos
% % Pioneer ***********************************************************
% %x
% figure;
% plot(time,Xd(1,:),'b--','LineWidth',sizeReal),hold on;
% plot(time,X(1,:),'r-','LineWidth',sizeReal);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
% xlim([0 time(end)]);
% 
% lgp1 = legend('$x_{1d}$','$x_{1r}$');
% lgp1.FontSize = 14;
% lgp1.Location = location;
% set(lgp1,'Interpreter','latex');
% grid on
% 
% %y
% figure;
% plot(time,Xd(2,:),'b--','LineWidth',sizeReal),hold on;
% plot(time,X(2,:),'r-','LineWidth',sizeReal);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
% xlim([0 time(end)]);
% 
% lgp2 = legend('$y_{1d}$','$y_{1r}$');
% lgp2.FontSize = 14;
% lgp2.Location = location;
% set(lgp2,'Interpreter','latex');
% grid on
% 
% % %z   % se for pioneer será sempre 0
% % figure;
% % plot(time,PXd(3,:),'b--','LineWidth',sizeReal),hold on;
% % plot(time,PX(3,:),'r-','LineWidth',sizeReal);
% % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
% % xlim([0 time(end)]);
% % 
% % lgp3 = legend('$z_{1d}$','$z_{1r}$');
% % lgp3.FontSize = 14;
% % lgp3.Location = location;
% % set(lgp3,'Interpreter','latex');
% % grid on
% 
% 
% % Drone *****************************************************************
% %x
% figure;
% plot(time,AXd(1,:),'b--','LineWidth',sizeReal),hold on;
% plot(time,AX(1,:),'r-','LineWidth',sizeReal);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
% xlim([0 time(end)]);
% 
% lgd1 = legend('$x_{2d}$','$x_{2r}$');
% lgd1.FontSize = 14;
% lgd1.Location = location;
% set(lgd1,'Interpreter','latex');
% grid on
% 
% %y
% figure;
% plot(time,AXd(2,:),'b--','LineWidth',sizeReal),hold on;
% plot(time,AX(2,:),'r-','LineWidth',sizeReal);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Dist\^ancia [m]','FontSize',sizeLabel,'interpreter','Latex');
% xlim([0 time(end)]);
% 
% lgd2 = legend('$y_{2d}$','$y_{2r}$');
% lgd2.FontSize = 14;
% lgd2.Location = location;
% set(lgd2,'Interpreter','latex');
% grid on
% 
% %z
% figure;
% plot(time,AXd(3,:),'b--','LineWidth',sizeReal),hold on;
% plot(time,AX(3,:),'r-','LineWidth',sizeReal);
% xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% ylabel('Altitude [m]','FontSize',sizeLabel,'interpreter','Latex');
% xlim([0 time(end)]);
% 
% lgd3 = legend('$z_{2d}$','$z_{2r}$');
% lgd3.FontSize = 14;
% lgd3.Location = location;
% set(lgd3,'Interpreter','latex');
% grid on
% 
% % #########################################################################
% % % % %% Com Subplots
% % % % % Velocidades ...............................................................
% % % % % Pioneer
% % % % figure;
% % % % subplot(321)
% % % % plot(time,PUd(1,:),'b--','LineWidth',sizeDesired), hold on;
% % % % plot(time,PU(1,:),'r','LineWidth',sizeReal);
% % % % legend({'$u_{d}$','$u_{r}$'},'FontSize',sizeLegenda,'Location','Southeast','Interpreter','latex');
% % % % grid on,xlim([0 time(end)]);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('Velocidade [m/s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % % title('(a)','Interpreter','latex');
% % % % 
% % % % subplot(322),
% % % % plot(time,PUd(2,:),'b--','LineWidth',sizeDesired), hold on;
% % % % plot(time,PU(2,:),'r','LineWidth',sizeReal)
% % % % legend({'$\omega_{d}$','$\omega_{r}$'},'FontSize',sizeLegenda,'Location','Southeast','Interpreter','latex');
% % % % grid on,xlim([0 time(end)]);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('Velocidade [rad/s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % % title('(b)','Interpreter','latex');
% % % % 
% % % % % ArDrone
% % % % % figure;
% % % % subplot(323)
% % % % plot(time,AUd(1,:),'b--','LineWidth',sizeDesired);hold on;
% % % % plot(time,AU(1,:),'r','LineWidth',sizeReal);
% % % % legend({'$\phi_{d}$','$\phi_{r}$'},'FontSize',sizeLegenda,'Location','Southeast','Interpreter','latex');
% % % % grid on,xlim([0 time(end)]);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('$\phi$ [rad]','FontSize',sizeLabel,'interpreter','Latex');
% % % % % title('(a)','Interpreter','latex');
% % % % 
% % % % subplot(324)
% % % % plot(time,AUd(2,:),'b--','LineWidth',sizeDesired);hold on;
% % % % plot(time,AU(2,:),'r','LineWidth',sizeReal);
% % % % legend({'$\theta_{d}$','$\theta_{r}$'},'FontSize',sizeLegenda,'Location','Southeast','Interpreter','latex');
% % % % grid on,xlim([0 time(end)]);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('$\theta$ [rad]','FontSize',sizeLabel,'interpreter','Latex');
% % % % % title('(b)','Interpreter','latex');
% % % % 
% % % % subplot(325)
% % % % plot(time,AUd(3,:),'b--','LineWidth',sizeDesired);hold on;
% % % % plot(time,AU(3,:),'r','LineWidth',sizeReal);
% % % % legend({'$\dot{z}_{d}$','$\dot{z}_{r}$'},'FontSize',sizeLegenda,'Location','Southeast','Interpreter','latex');
% % % % grid on,xlim([0 time(end)]);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('$\dot{z}$ [m/s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % % title('(c)','Interpreter','latex');
% % % % 
% % % % subplot(326)
% % % % plot(time,AUd(4,:),'b--','LineWidth',sizeDesired);hold on;
% % % % plot(time,-AU(4,:),'r','LineWidth',sizeReal);
% % % % legend({'$\dot{\psi}_{d}$','$\dot{\psi}_{r}$'},'FontSize',sizeLegenda,'Location','Southeast','Interpreter','latex');
% % % % grid on,xlim([0 time(end)]);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('$\dot{\psi}$ [rad/s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % % title('(d)','Interpreter','latex');
% % % % 
% % % % 
% % % % % Erro da formação ....................................................................
% % % % 
% % % % % Position
% % % % figure;
% % % % subplot(211)
% % % % plot(time,Qtil(1,:),'b--','LineWidth',sizeReal),hold on;
% % % % plot(time,Qtil(2,:),'r-','LineWidth',sizeReal);
% % % % plot(time,Qtil(4,:),'g-.','LineWidth',sizeReal);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('Erro [m]','FontSize',sizeLabel,'interpreter','Latex');
% % % % xlim([0 time(end)]);
% % % % % title('(a)','Interpreter','latex');
% % % % 
% % % % lg3 = legend('$x_f$','$y_f$','$\rho_f$');
% % % % lg3.FontSize = 14;
% % % % lg3.Location = 'SouthEast';
% % % % set(lg3,'Interpreter','latex');
% % % % grid on;
% % % % 
% % % % 
% % % % % Angles
% % % % % figure;
% % % % subplot(212)
% % % % plot(time,Qtil(5,:),'b--','LineWidth',sizeReal),hold on;
% % % % plot(time,Qtil(6,:),'r-','LineWidth',sizeReal);
% % % % xlabel('Tempo [s]','FontSize',sizeLabel,'interpreter','Latex');
% % % % ylabel('Erro [rad]','FontSize',sizeLabel,'interpreter','Latex');
% % % % xlim([0 time(end)]);
% % % % 
% % % % lg4 = legend('$\alpha_f$','$\beta_f$');
% % % % lg4.FontSize = 14;
% % % % lg4.Location = 'SouthEast';
% % % % set(lg4,'Interpreter','latex');
% % % % grid on
% 
% 
% % % %% Separate trajectory in 3 parts
% % % % Total de 3 voltas, a ideia é plotar cada volta separadamente
% % % lap = floor(4/12*length(AX));
% % % sizeDot = [10 2];
% % % % primeira volta .....................................
% % % figure;
% % % subplot(131)
% % % % percurso desejado
% % % pd1 = plot3(PXd(1,1:lap),PXd(2,1:lap),PXd(3,1:lap)+h,'r--','LineWidth',sizeReal); hold on;
% % % pd2 = plot3(AXd(1,1:lap),AXd(2,1:lap),AXd(3,1:lap),'b--','LineWidth',sizeReal);
% % % 
% % % % percurso realizado
% % % pr1 = plot3(PX(1,1:lap),PX(2,1:lap),PX(3,1:lap)+h,'r-','LineWidth',sizeReal); hold on;
% % % pr2 = plot3(AX(1,1:lap),AX(2,1:lap),AX(3,1:lap),'b-','LineWidth',sizeReal);
% % % 
% % % % Ponto final do trecho
% % % plot3(PX(1,lap),PX(2,lap),PX(3,lap)+h,'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% % % plot3(AX(1,lap),AX(2,lap),AX(3,lap),'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % 
% % % % Initial positions
% % % pi1 = plot3(PX(1,1),PX(2,1),PX(3,1)+h,'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % pi2 = plot3(AX(1,1),AX(2,1),AX(3,1),'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % 
% % % axis([-2 2 -2 2 0 1.5])
% % % grid on
% % % box on
% % % 
% % % % leg11 = legend([ pi1 pd1 pd2 pr1 pr2],{'Posi\c{c}\~{a}o inicial','Trajet\''{o}ria desejada Pioneer 3-DX', ...
% % % %     'Trajet\''{o}ria desejada AR.Drone 2.0','Trajet\''{o}ria Pioneer 3-DX','Trajet\''{o}ria AR.Drone 2.0'});
% % % % leg11.FontSize = sizeLegenda;
% % % % leg11.Location = location;
% % % % set(leg11,'Interpreter','latex');
% % % 
% % % % segunda volta ............................................
% % % % figure
% % % subplot(132)
% % % % percurso desejado
% % % plot3(PXd(1,lap:2*lap),PXd(2,lap:2*lap),PXd(3,lap:2*lap)+h,'r--','LineWidth',sizeReal); hold on;
% % % plot3(AXd(1,lap:2*lap),AXd(2,lap:2*lap),AXd(3,lap:2*lap),'b--','LineWidth',sizeReal);
% % % 
% % % % percurso realizado
% % % plot3(PX(1,lap:2*lap),PX(2,lap:2*lap),PX(3,lap:2*lap)+h,'r-','LineWidth',sizeReal); hold on;
% % % plot3(AX(1,lap:2*lap),AX(2,lap:2*lap),AX(3,lap:2*lap),'b-','LineWidth',sizeReal);
% % % 
% % % % Ponto inicial do trecho
% % % plot3(PX(1,lap),PX(2,lap),PX(3,lap)+h,'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% % % plot3(AX(1,lap),AX(2,lap),AX(3,lap),'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % 
% % % % Ponto final do trecho
% % % plot3(PX(1,2*lap),PX(2,2*lap),PX(3,2*lap)+h,'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% % % plot3(AX(1,2*lap),AX(2,2*lap),AX(3,2*lap),'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % 
% % % axis([-2 2 -2 2 0 1.5])
% % % grid on
% % % box on
% % % 
% % % % terceira volta ...............................................
% % % % figure
% % % subplot(133)
% % % % percurso desejado
% % % ptd1 = plot3(PXd(1,2*lap:end),PXd(2,2*lap:end),PXd(3,2*lap:end)+h,'r--','LineWidth',sizeReal); hold on;
% % % ptd2 = plot3(AXd(1,2*lap:end),AXd(2,2*lap:end),AXd(3,2*lap:end),'b--','LineWidth',sizeReal);
% % % 
% % % % percurso realizado
% % % ptr1 = plot3(PX(1,2*lap:end),PX(2,2*lap:end),PX(3,2*lap:end)+h,'r-','LineWidth',sizeReal); hold on;
% % % ptr2 = plot3(AX(1,2*lap:end),AX(2,2*lap:end),AX(3,2*lap:end),'b-','LineWidth',sizeReal);
% % % 
% % % % Ponto inicial do trecho
% % % plot3(PX(1,2*lap),PX(2,2*lap),PX(3,2*lap)+h,'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% % % plot3(AX(1,2*lap),AX(2,2*lap),AX(3,2*lap),'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % 
% % % 
% % % % Final positions
% % % pend1 = plot3(PX(1,end),PX(2,end),PX(3,end),'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % pend2 = plot3(AX(1,end),AX(2,end),AX(3,end),'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% % % 
% % % axis([-2 2 -2 2 0 1.5])
% % % grid on
% % % box on
% % % 
% % % % legenda
% % % leg12 = legend([pend1 ptd1 ptd2 ptr1 ptr2],{'Posi\c{c}\~{a}o final','Trajet\''{o}ria desejada Pioneer 3-DX','Trajet\''{o}ria desejada AR.Drone 2.0',...
% % %     'Trajet\''{o}ria Pioneer 3-DX','Trajet\''{o}ria AR.Drone 2.0'});
% % % leg12.FontSize = sizeLegenda;
% % % leg12.Location = location;
% % % set(leg12,'Interpreter','latex');
% % % 
% %% Separate trajectory in 2 parts
% % Total de 3 voltas, a ideia é plotar cada volta separadamente
% lap = floor(4/8*length(AX));
% sizeDot = [10 2];
% % primeira volta .....................................
% figure;
% subplot(121)
% % percurso desejado
% pd1 = plot3(Xd(1,1:lap),Xd(2,1:lap),Xd(3,1:lap)+h,'r--','LineWidth',sizeReal); hold on;
% pd2 = plot3(AXd(1,1:lap),AXd(2,1:lap),AXd(3,1:lap),'b--','LineWidth',sizeReal);
% 
% % percurso realizado
% pr1 = plot3(X(1,1:lap),X(2,1:lap),X(3,1:lap)+h,'r-','LineWidth',sizeReal); hold on;
% pr2 = plot3(AX(1,1:lap),AX(2,1:lap),AX(3,1:lap),'b-','LineWidth',sizeReal);
% 
% % Ponto final do trecho
% pend1 = plot3(X(1,lap),X(2,lap),X(3,lap)+h,'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% pend2 = plot3(AX(1,lap),AX(2,lap),AX(3,lap),'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% 
% % Initial positions
% pi1 = plot3(X(1,1),X(2,1),X(3,1)+h,'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% pi2 = plot3(AX(1,1),AX(2,1),AX(3,1),'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% 
% axis([-2 2 -2 2 0 1.5])
% grid on
% box on
% 
% % leg11 = legend([ pi1 pd1 pd2 pr1 pr2],{'Posi\c{c}\~{a}o inicial','Trajet\''{o}ria desejada Pioneer 3-DX', ...
% %     'Trajet\''{o}ria desejada AR.Drone 2.0','Trajet\''{o}ria Pioneer 3-DX','Trajet\''{o}ria AR.Drone 2.0'});
% % leg11.FontSize = sizeLegenda;
% % leg11.Location = location;
% % set(leg11,'Interpreter','latex');
% 
% % segunda volta ............................................
% % figure
% subplot(122)
% % percurso desejado
% plot3(Xd(1,lap:2*lap),Xd(2,lap:2*lap),Xd(3,lap:2*lap)+h,'r--','LineWidth',sizeReal); hold on;
% plot3(AXd(1,lap:2*lap),AXd(2,lap:2*lap),AXd(3,lap:2*lap),'b--','LineWidth',sizeReal);
% 
% % percurso realizado
% plot3(X(1,lap:2*lap),X(2,lap:2*lap),X(3,lap:2*lap)+h,'r-','LineWidth',sizeReal); hold on;
% plot3(AX(1,lap:2*lap),AX(2,lap:2*lap),AX(3,lap:2*lap),'b-','LineWidth',sizeReal);
% 
% % Ponto inicial do trecho
% plot3(X(1,lap),X(2,lap),X(3,lap)+h,'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% plot3(AX(1,lap),AX(2,lap),AX(3,lap),'k^','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% 
% % Ponto final do trecho
% plot3(X(1,2*lap),X(2,2*lap),X(3,2*lap)+h,'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2)); hold on;
% plot3(AX(1,2*lap),AX(2,2*lap),AX(3,2*lap),'ksq','MarkerSize',sizeDot(1),'LineWidth',sizeDot(2));
% 
% axis([-2 2 -2 2 0 1.5])
% grid on
% box on
% 
% % legenda
% leg12 = legend([pi1 pend1 pd1 pd2 pr1 pr2],{'Posi\c{c}\~{a}o inicial',...
%     'Posi\c{c}\~{a}o final','Trajet\''{o}ria desejada Pioneer 3-DX',...
%     'Trajet\''{o}ria desejada AR.Drone 2.0',...
%     'Trajet\''{o}ria Pioneer 3-DX','Trajet\''{o}ria AR.Drone 2.0'});
% leg12.FontSize = sizeLegenda;
% leg12.Location = location;
% set(leg12,'Interpreter','latex');
% 
