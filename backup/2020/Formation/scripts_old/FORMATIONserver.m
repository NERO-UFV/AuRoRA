...
if length(Rede.pMSG.getFrom)>1
    ...
    try
        ...
        F.pPos.tempX = Rede.pMSG.getFrom{2}(15:26);  % captura postura X de cada robô
    ...
    % Cálculo das Variáveis da formação inicial
    F.mCalculaFormacao;
...
%             disp('entrou no if');
            ...
            try
                % Povoa variáveis do robo
                F.pPos.tempX = Rede.pMSG.getFrom{2}(15:26);  % captura postura X do robô
                %    Postura dos robôs da formação: [x1 y1 z1 rho1 theta1 psi1 x2 y2 z2 rho2 theta2 psi2]
                F.pPos.X = [P{ID}.pPos.X(1:6);F.pPos.tempX(1:6)];
            catch
                disp('Dados da rede incompletos.');
...
%             Rede.mSendMsg(F);       % envia dados da formação para rede
