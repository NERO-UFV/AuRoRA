function [] = rPlotPositionVar(tmax)
...
%     filename = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cDados Simu%cPosition%csimu_atraso_variavel%d.mat', '\', '\', '\', '\', '\', '\', '\', '\', '\',j);
    % Trajetória (carrega dados e salva figura)
    if j == 1
...
%     name_fig = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cFigs%cfigPos Atraso_Variável%d', '\', '\', '\', '\', '\', '\', '\', '\', j);
