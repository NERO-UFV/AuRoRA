...
SP(3).OuterPosition = [0.6845 0.33 0.3 0.33];
SP(4).OuterPosition = [0.7 0.0 0.3 0.33];
...
tmax = 60; % Simulation time in seconds
...
while toc(t) < tmax
    if toc(tc) > ta
        ...
        etaa = Ganhos.kt1*tanh(Ganhos.kt2*(qd(7)-q(7))) + Ganhos.kt3*tanh(Ganhos.kt4*(dqd(7)-dq(7)));
        etab = Ganhos.kp1*tanh(Ganhos.kp2*(qd(8)-q(8))) + Ganhos.kp3*tanh(Ganhos.kp4*(dqd(8)-dq(8)));
        ...
        % Sinal de controle
        eta = 0.1*diag([1 1 10 100 100 100 10 10])*(qd-q) + 0.1*diag([1 1 1 15 15 10 1 1])*(dqd-dq); % + ...
...
hold on
plot(QQ(:,end),QQ(:,1),'-r')
legend('Real','Goal','Interpreter','latex')
...
hold on
plot(QQ(:,end),zeros(size(QQ,1),1),'-r')
legend('Real','Goal','Interpreter','latex')
title('$\alpha$  vs t','Interpreter','latex')
ylabel('{\alpha} [°]','FontSize',16)
xlabel('Time [s]','FontSize',16)
subplot(2,1,2)
plot(QQ(:,end),rad2deg(QQ(:,18)),'--k','LineWidth',1.2)
hold on
plot(QQ(:,end),zeros(size(QQ,1),1),'-r')
legend('Real','Goal','Interpreter','latex')
