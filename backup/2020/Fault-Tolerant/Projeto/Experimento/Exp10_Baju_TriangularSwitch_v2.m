...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    THETAfd = -BETAfi/2+BETAfi/2*(toc(T)-T_MAX/4)/(T_MAX/4);
    Qd{1}(:,2) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  RAIO(1)/cos(THETAfd); RAIO(1)/cos(THETAfd+BETAfi); BETAfi];

    % Missão 2 - Drone 1 afasta para Drone 2 levantar voo
    THETAfd = -BETAfi*toc(T)/(T_MAX/4);
    Qd{2}(:,1) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  RAIO(1)/cos(THETAfd); RAIO(1)/cos(THETAfd+BETAfi); BETAfi];
    ...
    Qd{3}(:,1) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  RAIO(1)/cos(THETAfd); .8+(RAIO(1)/cos(THETAfd+BETAfi)-.8)*toc(T)/(T_MAX/4); BETAfi];
              
    % Missão 4 - Formação atua para deixar o Drone 1 em cima do Pioneer
    THETAfd = -BETAfi+BETAfi*toc(T)/(T_MAX/4);
    Qd{4}(:,1) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  RAIO(1)/cos(THETAfd); RAIO(1)/cos(THETAfd+BETAfi); BETAfi];
    ...
    Qd{5}(:,1) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  RAIO(1)/cos(THETAfd)+(H_LAND-RAIO(1)/cos(THETAfd))*toc(T)/(T_MAX/4); RAIO(1)/cos(THETAfd+BETAfi); BETAfi];
