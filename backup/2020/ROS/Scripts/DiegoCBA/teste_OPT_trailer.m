...
%             P.pPos.Xd([7 8]) = [ww*rX*cos(ww*t_atual);-ww*rY*sin(ww*t_atual)];
            ...
            P2.pPos.Xd([1 2]) = [rX*sin(ww*t_atual)-0.49;-rY*cos(ww*t_atual)];
            P2.pPos.Xd([7 8]) = [ww*rX*cos(ww*t_atual);ww*rY*sin(ww*t_atual)];
...
%           angle2OPT_hist = [angle2OPT_hist P3.pPos.X(6)];
          t_hist = [t_hist toc(t)];
          x_t1_hist = [x_t1_hist P2.pPos.X(1:6)];
...
save(fullname,'xd_hist','x_hist','angle1_hist','angle2_hist','angle1OPT_hist','angle2OPT_hist','t_hist')
