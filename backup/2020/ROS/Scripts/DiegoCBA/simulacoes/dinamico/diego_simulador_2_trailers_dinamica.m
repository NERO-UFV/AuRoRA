...
%         Vd_trajetoria([1 2]) = [-ww*rX*cos(ww*t_atual + phase);-2*ww*rY*cos(2*ww*t_atual + phase)];
       ...
       %Saturação
        ...
        %         Cinematica direta com 2 trailers - testes cinematico
        ...
        %%% transmissao velocidade direta do robo para o trailer1 cinematico
%         U_trailer1(1) = U_robo(1)*cos(theta_1) + (U_robo(3))*(L0)*sin(theta_1);
%         U_trailer1(3) = U_robo(1)*sin(theta_1)/(L1) - (U_robo(3))*(L0)*cos(theta_1)/(L1);
%         U_trailer1(2) = 0;
...
%         dX_robo([1 2 4]) = K_inv_robo\U_robo([1 2 3]);   %%%% teste cinematico
