...
%         A_din = [Theta(1) 0; 0 Theta(2)];
%           
%         %%% habilita sinal de controle dinamico
%         U_robo([1 3]) = A_din*[delta_1;delta_2] + G*Theta;
        
%         if k > 1000
%         U_robo(isnan(U_robo))=0.1;
%         
%         if abs(dX_robo(5)) > 0.1
%             dX_robo(5) = 0.1*sign(dX_robo(5));
%         end
%         
%         if abs(dX_robo(5)) < -0.1
%             dX_robo(5) = 0.1*sign(dX_robo(5));
%         end
%         
%         if abs(dX_robo(6)) > 0.1
%             dX_robo(6) = 0.1*sign(dX_robo(6));
%         end
%         
%         if abs(dX_robo(6)) < -0.1
%             dX_robo(6) = 0.1*sign(dX_robo(6));
