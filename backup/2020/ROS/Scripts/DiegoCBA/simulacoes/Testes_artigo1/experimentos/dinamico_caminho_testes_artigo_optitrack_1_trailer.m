...
Ve = 0.25; % Velocidade desejada no caminho
...
%         end
       ...
       U_robo_real([1 2 3]) = [v0_c 0 w0_c];   %%%% sinal de controle cinematico para o robo1
        ...
        velocidade_real_robo = -sqrt(velocidades_robo(1)^2+velocidades_robo(2)^2);
         ...
         %%%% -------------   compensador dinâmico real ---------------
          ...
          G_real = [0 0 -P.pPos.Xc(12)^2 velocidade_real_robo 0 0; ...
               0 0 0 0 P.pPos.Xc(12)*velocidade_real_robo P.pPos.Xc(12)];
          
          delta_1_real = U1_robo_real_ref + k_u*(U_robo_real(1) - velocidades_robo(1));
          delta_2_real = U3_robo_real_ref + k_w*(U_robo_real(3) - velocidades_robo());
...
%              
      ...
      velocidade_real_trailer1 = -sqrt(velocidades_trailer1(1)^2+velocidades_trailer1(2)^2);
      
      %%%%%%%%%%% ---------------------Calculo erro velocidade ---------------------
      erro_v_linear = abs(-Vd - velocidade_real_trailer1);
      erro_v_angular = abs(v_fi_real - P2.pPos.Xc(12));
      ...
      %%%%% ------------------------------------------------
        ...
        V_real_trailer1_hist = [V_real_trailer1_hist velocidade_real_trailer1];
...
%         grid on
%         plot(t_hist(1,:),sinal_controle_trailer_hist(2,:))
%         plot(t_hist(1,:),P2_real_X_hist(12,:))
%         axis([0 65 -2 2])
...
% % % %% 
% path = 'C:\Users\lai1u\Dropbox\AuRoRA 2018\ROS\Scripts\DiegoCBA\simulacoes\Testes_artigo1\experimentos\Dados_1_trailer_filmados\';
% filename = ['Diego_cinematico_lemniscata_desfavoravel_v_' num2str(Ve) '_Data_' datestr(now,30) '.mat'];
