...
while size(open,1) ~= 0
    ...
    for i = 1:size(suc,1)
        ...
        if sum(c(:,1) == closed(:,1)) == 0
            ...
            disp(atual)
            desejado = c;
            while toc(t) < t_transicao
                if toc(ta) > 0.1
                    ta = tic;
                    
                    P.pPos.Xda = P.pPos.Xd;
                    P.pPos.Xd(1:2) = [atual(2)+(desejado(2)-atual(2))*toc(t)/t_transicao;
                        atual(3)+(desejado(3)-atual(3))*toc(t)/t_transicao];
                    
                    P.pPos.Xd(7:8) = (P.pPos.Xd(1:2)-P.pPos.Xda(1:2))./toc(dt);
                    dt = tic;
