...
tmax = 40; % Tempo Simulacao em segundos
kA = 0.1;
kw = 0.5;
  ...
  try
    while toc(t) < tz
        ...
        if toc(tc) > To            
            ...
            B.pSC.Ud(3) = VHref; % -v{k}(6)*ganho; % Velocidade Vertical [-1,1] (+) Eleva o drone
...
fileName = strcat('Calibracao_Const',toStringJSON(serialNumber),'.mat');
fullFileName = strcat(dataPath,fileName);
