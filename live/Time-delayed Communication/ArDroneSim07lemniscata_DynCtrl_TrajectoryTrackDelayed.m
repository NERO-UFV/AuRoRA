...
% Atraso máximo de 1s
for idAtraso = (0:67)  % Atraso entre 0 a 2 segundos!
...
%         fprintf(ArqTxt,'\n');
    ...
    if toc(tp) > inf
        ...
        A.mCADplot;
        Ar.mCADplot;
        plot3(xx(:,1),yy(:,1),zz(:,1),'-b') % realizada
        plot3(XX(:,1),XX(:,2),XX(:,3),'--r') % desejada
        drawnow
        hold on            
    end
...
% save('TrajIndDesempenhoL.mat','IAE_ITAE_IASC')
% load('TrajIndDesempenhoL.mat','IAE_ITAE_IASC')
...
VetorAtraso = [0, 34, 67, 167, 334];

rPlotPositionTraj(VetorAtraso, tmax);
%-----
...
rPlotPerformanceAnalysisGeneral(IAE_ITAE_IASC, tmax/20)
