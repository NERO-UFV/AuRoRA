function [] = rPlotPerformanceAnalysis(idAtraso, tmax)
%****** Função para plotar os principais gráficos ******%

for j = (1:2)
    
%     filename = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cDados Simu%cPosition%csimu_atraso_%d.mat', '\', '\', '\', '\', '\', '\', '\', '\', '\', idAtraso(j));
%     filename = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cDados Simu%cTrajectory Tracking%csimu_atraso_%d(Circunferência).mat', '\', '\', '\', '\', '\', '\', '\', '\', '\', idAtraso(j));
%     filename = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cDados Simu%cTrajectory Tracking%csimu_atraso_%d(Lemniscata).mat', '\', '\', '\', '\', '\', '\', '\', '\', '\', idAtraso(j));
    ...
    if j == 1
        ...
        name_fig = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cFigs%cfigTaj Atraso_VariávelID(Circunferência)', '\', '\', '\', '\', '\', '\', '\', '\');

    else
        name_fig = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cFigs%cfigTraj Atraso_VariávelID(Lemniscata)', '\', '\', '\', '\', '\', '\', '\', '\');
    ...
    fInd = figure('Name','Análise de Performance','NumberTitle','off');
    fInd.Position = [488 2 875 618]; 
    figure(fInd);

    subplot(311),plot(Arq.Hist(:,end),Arq.Hist(:,end-3), 'LineWidth', 1)
    if j == 1
        lgd = legend({'UAV$_{Actu}$','UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','southeast');
    else
        lgd = legend({'UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','northwest');
    end
    title(lgd,'IAE','FontWeight','bold');
    grid on
    axis([0 tmax round(min(Arq.Hist(:,end-3))-5) round(max(Arq.Hist(:,end-3))+5)])
    ...
    ylabel('IAE','FontSize',15,'FontWeight','bold','interpreter','Latex')

    subplot(312),plot(Arq.Hist(:,end),Arq.Hist(:,end-2), 'LineWidth', 1)
    if j == 1
        lgd1 = legend({'UAV$_{Actu}$','UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','southeast');
    else
        lgd1 = legend({'UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','northwest');
    end
%     lgd1 = legend({'UAV$_{Actu}$','UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','northwest');
    title(lgd1,'ITAE','FontWeight','bold');
    grid on
    axis([0 tmax round(min(Arq.Hist(:,end-2))-5) round(max(Arq.Hist(:,end-2))+5)])
    % xlabel('$t$ [s]','FontSize',12,'FontWeight','bold','interpreter','Latex')
    ylabel('ITAE','FontSize',15,'FontWeight','bold','interpreter','Latex')

    subplot(313),plot(Arq.Hist(:,end),Arq.Hist(:,end-1), 'LineWidth', 1)
    if j == 1
        lgd2 = legend({'UAV$_{Actu}$','UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','southeast');
    else
        lgd2 = legend({'UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','northwest');
    end
%     lgd2 = legend({'UAV$_{Actu}$','UAV$_{delayed}$'},'FontSize',12,'interpreter','latex','Location','northwest');
    title(lgd2,'IASC','FontWeight','bold');
    grid on
    axis([0 tmax round(min(Arq.Hist(:,end-1))-5) round(max(Arq.Hist(:,end-1))+5)])
    xlabel('$t$[$s$]','FontSize',15,'FontWeight','bold','interpreter','Latex')
    ylabel('IASC','FontSize',15,'FontWeight','bold','interpreter','Latex')


%     name_fig = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cFigs%cfigPos Atraso_%d_Amostras Indices_Desempenho', '\', '\', '\', '\', '\', '\', '\', '\', idAtraso(j));
%     name_fig = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cFigs%cfigTraj atraso_%d(Lemniscata)Indices_Desempenho', '\', '\', '\', '\', '\', '\', '\', '\', idAtraso(j));
%     name_fig = sprintf('C:%cUsers%cleo_j%cDropbox%cDropbox%cAuRoRA 2018%cTime-delayed Communication%cFigs%cfigTraj atraso_%d(Circunferência)Indices_Desempenho', '\', '\', '\', '\', '\', '\', '\', '\', idAtraso(j));
    saveFigure(fInd, name_fig);     % Salva a figura .fig
    saveas(fInd, name_fig, 'epsc'); % Salva a figura .eps
    saveas(fInd, name_fig, 'pdf');  % Salva a figura .pdf
    saveas(fInd, name_fig, 'png');  % Salva a figura .png
end
%-------------------------------------------------------------------------
