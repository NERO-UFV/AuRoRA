...
% Atraso máximo de 1s
for idAtraso = (0:334/2)  % Atraso entre 0 a 5 segundos!

    
% Nome arquivo .mat
% idAtraso = 10;
% mkdir('C:\Users\leo_j\Dropbox\Dropbox\AuRoRA 2018\Time-delayed Communication\Dados Simu','Position');
...
XX = [A.pPos.Xd' A.pPos.X' A.pSC.Ud' IAE ITAE IASC toc(t)];
kk = 1;
...
%         IASC_Ar = IASC_Ar + (abs(ddXAr(1)) + abs(ddXAr(2)) + abs(ddXAr(3)))*dt;
        ...
        % Histórico de dados:
        XX = [XX; [A.pPos.Xd' A.pPos.X' A.pSC.Ud' IAE ITAE IASC toc(t)]];      
        kk = kk + 1;
...
IAE_ITAE_IASC = [IAE_ITAE_IASC; IAE ITAE IASC idAtraso];
% save('PosIndDesempenho.mat','IAE_ITAE_IASC')
load('PosIndDesempenho.mat','IAE_ITAE_IASC')
...
% VetorAtraso = [0, 34, 67, 167];

VetorAtraso = [1, 4, 10, 15];
tmax = 40; % Tempo Simulação em segundos

rPlotPosition(VetorAtraso, tmax);

% % fPos = figure('Name','Posicionamento ArDrone','NumberTitle','off');
% % fPos.Position = [403 2 960 682]; 
% % figure(fPos);
% % 
...
%%----- IAE, ITAE e IASC para Atraso Específico:
VetorAtraso = [0, 10, 15];
rPlotPerformanceAnalysis(VetorAtraso, tmax)

% % fInd = figure('Name','Posicionamento ArDrone','NumberTitle','off');
% % fInd.Position = [403 2 960 682]; 
% % figure(fInd);
% % 
...
%%----- IAE, ITAE e IASC geral (efeitos de atraso crescente):
rPlotPerformanceAnalysisGeneral(IAE_ITAE_IASC, tmax)
