%% Testar aterrissar e posteriormente decolar durante controle de posição
% O teste é para verificar os erros de conectividade que o drone vem apresentando
% neste tipo de experimento. Para tanto, enquanto estiver no chão, nenhum
% sinal de controle é enviado ao drone
...
tsim    = size(Xd,1)*(time); % Simulation duration [s]
...
while toc(t) < tsim
    if toc(tc) > 1/30
        ...
        if cont<=size(Xd,1) %to not exceed desired positions matrix index
           % set new desired coordinates
        ...
        if toc(t) > 5*tsim/size(Xd,1)        % taking off
            if A.pCom.cStatus(32)==0           % 0 = landed
                A.rTakeOff;                    % takeoff again
            end
        elseif toc(t) > 3*tsim/size(Xd,1)-1  % landing
            if A.pCom.cStatus(32)==1           % 1 = flying state  
                A.rLand;
            end
        end
        ...
        % Control
        A = cUnderActuatedController(A);  % "dinamic compensation" 
        ...
        % Send control signals (only if in flying state) 
