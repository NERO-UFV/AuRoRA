...
% cd('DataFiles')
% cd('Log_Optitrack')
Arq = fopen(['FL3d_PositionUtimateNewControllers' NomeArq '.txt'],'w');
% cd(PastaAtual)
dados = [];

%% Robot/Simulator conection
% A.pPar.ip = '192.168.1.30';    % ar.drone IP (use ipconfig to verify)
A.pPar.ip = '192.168.1.40';    % ar.drone IP (use ipconfig to verify)
A.rConnect;
...
time = 14;    % time to change desired positions [s]
...
% %         % go to the next position if x and y position errors are small
% %         if (LF.pPos.Qtil(1)<0.06) && (LF.pPos.Qtil(2)<0.06)
% %             cont = cont + 1;
% %             pause(1);    % time for drone stabilize if necessary
% %         elseif (toc(t)> cont*time) && (cont <= size(Qd,1))  % define max time limit to change desired position
% %             cont = cont + 1;
% %         end
        ...
        elseif toc(t) > 3*tsim/size(Qd,1)-8    % Landing
