...
cd('DataFiles')
cd('Log_Optitrack')
Arq = fopen(['FL3d_TrajLemni&Elipse' NomeArq '.txt'],'w');
cd(PastaAtual)
...
while toc(t) < tsim 
    if toc(tc) > 1/30
        ...
        if toc(t) > tsim/nlap
            ...
            Qd(5)          = pi;
            % Positions
            LF.pPos.Qd(1)  = a*cos(w*ta);          % x
            LF.pPos.Qd(2)  = b*sin(w*ta);          % y
            LF.pPos.Qd(5)  = Qd(5) + P.pPos.X(6);  % alpha 
            % Velocities
            LF.pPos.dQd(1) = -a*w*sin(w*ta);       % dx 
            LF.pPos.dQd(2) = b*w*cos(w*ta);        % dy 
            LF.pPos.dQd(5) = 0;                    % dalpha 
            
        else
            % Lemniscata
            % Positions
            LF.pPos.Qd(1)  = a*sin(w*ta);         % x 
            LF.pPos.Qd(2)  = b*sin(2*w*ta);       % y 
            LF.pPos.Qd(5)  = Qd(5) + P.pPos.X(6); % alpha 
            % Velocities
            LF.pPos.dQd(1) = a*w*cos(w*ta);       % dx 
            LF.pPos.dQd(2) = b*2*w*cos(2*w*ta);   % dy                   
        end
        ...
        %% Acquire sensors data (from optitrack)
        % Get network data
        Rede.mReceiveMsg;
