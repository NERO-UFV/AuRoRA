function rigidBody = getOptData_rmma(optData,rigidBody)
...
if optData.isTracked
    ...
    rigidBody.pPos.Xac = rigidBody.pPos.Xc;  % Save previous data for derivative
    ...
    if type(1) == 'P' || type(1) == 'p' || type(2) == 'P' || type(2) == 'P'
        % Position [m]
   ...
   dt = toc(rigidBody.pPar.ti);    % sample time
    
%      dt = rigidBody.pPar.Ts;    % sample time % adicionado por VALENTIM

    
    rigidBody.pPos.X(7:9)   = (rigidBody.pPos.X(1:3)-rigidBody.pPos.Xa(1:3))/dt;    % dX,dY,dZ
    rigidBody.pPos.Xc(7:9)   = (rigidBody.pPos.Xc(1:3)-rigidBody.pPos.Xac(1:3))/dt;    % dX,dY,dZ
    rigidBody.pPos.X(10:11) = (rigidBody.pPos.X(4:5)-rigidBody.pPos.Xa(4:5))/dt;  % phi,theta
    rigidBody.pPos.Xc(10:11) = (rigidBody.pPos.Xc(4:5)-rigidBody.pPos.Xac(4:5))/dt;  % phi,theta
    ...
    if abs(rigidBody.pPos.Xc(6) - rigidBody.pPos.Xac(6)) > pi                       % Yaw Test
        if rigidBody.pPos.Xac(6) < 0
            rigidBody.pPos.Xac(6) =  2*pi + rigidBody.pPos.Xac(6);
        else
            rigidBody.pPos.Xac(6) = -2*pi + rigidBody.pPos.Xac(6);
        end
    end
    rigidBody.pPos.X(12) = (rigidBody.pPos.X(6)-rigidBody.pPos.Xa(6))/dt;          % dPsi
    rigidBody.pPos.Xc(12) = (rigidBody.pPos.Xc(6)-rigidBody.pPos.Xac(6))/dt;          % dPsi
    rigidBody.pPar.ti = tic;
