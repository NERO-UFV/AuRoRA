...
%                 RLS.T0=2*ones(RLS.n,1); % Pacheco & vSA
        ...
        RLS.T(:,1) = RLS.T0 + RLS.P0*RLS.Y{1}*inv(eye(RLS.linhas_g) + RLS.Y{1}'*RLS.P0*RLS.Y{1})*RLS.e(:,1); %eye(1,1) 1 indica a qtd de linhas do regressor (matriz G)
        ...
        function parametros_estimados = atualiza(RLS,uk,Gk)
            ...
            RLS.T(:,k) = RLS.T(:,k-1) + RLS.P*RLS.Y{k}*inv(alpha*eye(RLS.linhas_g) + RLS.Y{k}'*RLS.P*RLS.Y{k})*RLS.e(:,k);
