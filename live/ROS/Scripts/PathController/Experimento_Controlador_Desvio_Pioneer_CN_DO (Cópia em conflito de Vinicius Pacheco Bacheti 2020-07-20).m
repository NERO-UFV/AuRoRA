...
Ve = 0.5; % Velocidade desejada no caminho
    ...
    while t_max > toc(t)
        if toc(t_c) > T_c
            ...
            if obs_ativo_dist < dist_detecta_obs && flag_desvio == 0 && flag_desvio_acabando == 0
                ...
                ang_obs_inicio = atan2(C_normal(2,ind_desvio_inicio) - obs_ativo(2),C_normal(1,ind_desvio_inicio) - obs_ativo(1))*180/pi;
            ...
            if flag_desvio == 0 && flag_desvio_acabando == 1
                if norm(X1(1:2)-obs_ativo(1:2)) > dist_detecta_obs
                    flag_desvio_acabando = 0;
                end
            end
...
%   writerObj = VideoWriter('Desvio_Caminho_Final_ObsMudando2.mp4','MPEG-4');
