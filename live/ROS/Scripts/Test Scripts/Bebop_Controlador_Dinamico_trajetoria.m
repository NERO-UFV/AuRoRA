...
% xd = [3 5 1 0]';
% syms t_sym
% xdt = [2*cos(t_sym) 2*sin(t_sym) 1 0];
% xdpt = diff(xdt);
% xd2pt = diff(xdpt);
xdponto = [0 0 0 0]';
...
mult = 2;
mult2 = 0.1;
xd = [mult*cos(mult2*toc(t)) mult*sin(mult2*toc(t)) 1 0]';
% xd = [-3 5 4 0]';
% xd = [mult*cos(mult2*toc(t))*sqrt(2*cos(2*mult2*toc(t))) mult*sin(mult2*toc(t))*sqrt(2*cos(2*mult2*toc(t))) 1 0]';
xdold = xd;
xdp = (xd-xdold)/To;
xdpold = xdp;
xd2p = (xdp-xdpold)/To;
...
%         xd = [mult*cos(mult2*toc(t))*sqrt(2*cos(2*mult2*toc(t))) mult*sin(mult2*toc(t))*sqrt(2*cos(2*mult2*toc(t))) 1 0]';
        xdp = (xd-xdold)/dt;
%         xdp = [-mult2*mult*sin(mult2*toc(t)) mult2*mult*cos(mult2*toc(t)) 0 0]';
        xd2p = (xdp-xdpold)/dt;
        
        % Controlador Dinamico Bebop 2
        B.pPos.X(12) = (B.pPos.X(6) - pPosAnt)/dt;
        pPosAnt = B.pPos.X(6);
        ...
        %             v = kp*Xtil + kd*Xtilponto;
        v = xd2p + kp*Xtil + kd*Xtilponto;
