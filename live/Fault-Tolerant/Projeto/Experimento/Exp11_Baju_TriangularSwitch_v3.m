...
% Ganhos formação
TF.pPar.K1 = .8*diag([.6  .6  1   1  4 2.2 2.2 2.2 1]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    Qd{1}(:,1) = [Xfi; Yfi; Zfi;
                  ...
                  RAIO(1)/cos(THETAfd); RAIO(1)/cos(THETAfd+BETAfi); BETAfi];

    R2d = RAIO(2)/2+RAIO(2)/2*(toc(T)-T_MAX/4)/(T_MAX/4); 
    THETAfd = -atan2((RAIO(2)-R2d),RAIO(1));
    Pfd = sqrt((RAIO(2)-R2d)^2+RAIO(1)^2);
    Qfd = sqrt(R2d^2+RAIO(1)^2);
    Qd{1}(:,2) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  Pfd; Qfd; acos((Pfd^2+Qfd^2-RAIO(2)^2)/(2*Pfd*Qfd))];

    % Missão 2 - Drone 1 afasta para Drone 2 levantar voo
    R2d = RAIO(2)-RAIO(2)*toc(T)/(T_MAX/4); 
    THETAfd = -atan2((RAIO(2)-R2d),RAIO(1));
    Pfd = sqrt((RAIO(2)-R2d)^2+RAIO(1)^2);
    Qfd = sqrt(R2d^2+RAIO(1)^2);
    Qd{2}(:,1) = [Xfi; Yfi; Zfi;
    ...
    % Missão 5 - Drone 1 pousa em cima do Pioneer
    R2d = RAIO(2); 
    THETAfd = THETAfi;
    Pfd = RAIO(1)+(H_LAND-RAIO(1))*toc(T)/(T_MAX/4);
    Qfd = sqrt(R2d^2+RAIO(1)^2);
    Qd{5}(:,1) = [Xfi; Yfi; Zfi;
                  THETAfd; PHIfi; PSIfi; 
                  Pfd; Qfd; BETAfi];
