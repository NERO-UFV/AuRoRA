...
%     Qd{1}{2}(:,1) = [rx*cos(W*toc(T{1})+alphatraj); ry*sin(W*toc(T{1})+alphatraj); Zfi(1);
%                     THETAfi; PHIfi; PSIfi;
%                     Pfi; Qfi; BETAfi];
                
    % Missão 3 - Pioneer 2 realiza a missão
    Qd{1}{2}(:,1) = [Xfd(1)+(Xfi(2)-Xfd(1))*toc(T{1})/(T_MAX*2); Yfd(1)+(Yfi(2)-Yfd(1))*toc(T{1})/(T_MAX*2); Zfi(1);
                    THETAfi; PHIfi; PSIfi;
                    Pfi; Qfi; BETAfi];
    
    % Missão 4 - Pioneer 2 retorna para o ponto inicial do Pioneer 1
    
    % DEPOIS
    
    % Missão 5 - Pioneer 2 e 3 andam orientado para o eixo y
