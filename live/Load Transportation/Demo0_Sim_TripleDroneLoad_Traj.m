...
% gains = [1 2 1 2 2 15;
%          2 13 2 15 1 5];
gains = [2 5 2 5 2 15; 3 13 3 15 1 5];
...
A{1}.pPar.uSat(1:2) = A{1}.pPar.uSat(1:2)*1;
A{1}.pPar.uSat(3) = A{1}.pPar.uSat(3)*1;
A{1}.pPar.uSat(4) = A{1}.pPar.uSat(4)*1;
...
% Leminiscata Variables
la = 1;
b = 1;
w = .012;

dej(1) = plot3(L{1}.pPos.Xd(1),L{1}.pPos.Xd(2),L{1}.pPos.Xd(3),'o');
...
while toc(t) < tsim
    if toc(tc) > A{1}.pPar.Ts
       ...
       if toc(t) > 10
        tt = toc(t);
        L{1}.pPos.Xd(1) = la*sin(2*pi*w*(tt-10));       % x
        L{1}.pPos.Xd(2) = b*sin(2*pi*2*w*(tt-10));      % y
        L{1}.pPos.Xd(3) = .5 +.5*sin(2*pi*w*(tt-10));   % z
        
        A{1}.pPos.Xd(7) = la*2*pi*w*cos(2*pi*w*tt);     % dx
        A{2}.pPos.Xd(7) = la*2*pi*w*cos(2*pi*w*tt);     % dx
        A{3}.pPos.Xd(7) = la*2*pi*w*cos(2*pi*w*tt);     % dx
        
        A{1}.pPos.Xd(8) = b*2*pi*2*w*cos(2*pi*2*w*tt); % dy
        A{2}.pPos.Xd(8) = b*2*pi*2*w*cos(2*pi*2*w*tt); % dy
        A{3}.pPos.Xd(8) = b*2*pi*2*w*cos(2*pi*2*w*tt); % dy
%         L{1}.pPos.Xd(9) = 0;
...
%     disp(L{1}.pPos.X(1:3)')
    if toc(tp) > 0.05
        ...
        dej(1).XData = L{1}.pPos.Xd(1); dej(1).YData = L{1}.pPos.Xd(2); dej(1).ZData = L{1}.pPos.Xd(3);
        drawnow
    end
end
disp('Fim!')
...
figure(1)
n = 500;
plot3(XX{1}(1,1:end-n)', XX{1}(2,1:end-n)',XX{1}(3,1:end-n)','r--'); hold on
plot3(XX{1}(13,1:end-n)',XX{1}(14,1:end-n)',XX{1}(15,1:end-n)','r-'); 

plot3(XX{2}(1,1:end-n)', XX{2}(2,1:end-n)', XX{2}(3,1:end-n)','b--'); hold on
plot3(XX{2}(13,1:end-n)',XX{2}(14,1:end-n)',XX{2}(15,1:end-n)','b-'); 

plot3(XX{3}(1,1:end-n)', XX{3}(2,1:end-n)', XX{3}(3,1:end-n)','k--','Color',[1 .9 0]); hold on
plot3(XX{3}(13,1:end-n)',XX{3}(14,1:end-n)',XX{3}(15,1:end-n)','-','Color',[1 .9 0]); 

plot3(XX{4}(1,1:end-n)', XX{4}(2,1:end-n)', XX{4}(3,1:end-n)','--','Color',[0 0.45 0.74]); hold on
plot3(XX{4}(13,1:end-n)',XX{4}(14,1:end-n)',XX{4}(15,1:end-n)','-','Color',[0 0.45 0.74]);
grid on
