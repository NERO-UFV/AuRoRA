...
gains1 = [0.15    2.00    0.15   2.00   08    2.00 ;  1   15   1   15   1   1 ; 0 1 0 1 1 1];
gains2 = [0.25    2.00    0.25   2.00   10    2.00 ;  1   15   1   15   1   1 ; 0 1 0 1 1 1];
...
while toc(t_run) < T_exp
    ...
    if toc(t_run) > T_run 
        ...
        if toc(t_task) > (T_task + 5) & i_task == 0
           t_task = tic;
           i_task = i_task + 1;
           barL.pPos.Qd = [.5 0 .6 deg2rad(180) deg2rad(0) barL.pPar.L]' ;
           disp('Tarefa 2')
        end
        ...
        if toc(t_task) > T_task & i_task == 1
           t_task = tic;
           i_task = i_task + 1;
           barL.pPos.Qd = [.5 0 .6 deg2rad(180) deg2rad(20) barL.pPar.L]';
           disp('Tarefa 3')
        end
        ...
        if toc(t_task) > T_task & i_task == 3
           t_task = tic;
           i_task = i_task + 1;
           barL.pPos.Qd = [.5 0 .6 deg2rad(270) deg2rad(20) barL.pPar.L]';
           disp('Tarefa 4')
        end
        ...
        if toc(t_task) > T_task & i_task == 4
           t_task = tic;
           i_task = i_task + 1;
           barL.pPos.Qd = [.5 0 .6 deg2rad(270) deg2rad(0) barL.pPar.L]';
           disp('Tarefa 5')
        end
...
% %         
        ...
        A{1} = cUnderActuatedControllerDaniel(A{1},gains1);
        A{2} = cUnderActuatedControllerDaniel(A{2},gains2);
        ...
        % Coleta de dados
        data = [data; A{1}.pPos.Xd' A{1}.pPos.X' A{1}.pPos.Xtil' A{1}.pPar.D(1:3)' A{2}.pPos.Xd' A{2}.pPos.X' A{2}.pPos.Xtil' A{2}.pPar.D(1:3)' L{1}.pPos.X' L{2}.pPos.X' toc(t_exp) A{1}.pPos.intXtil' A{2}.pPos.intXtil'];
%                       1:12             13:24        25:36           37:39            40:51          52:63        64:75           76:78             79:90        91:102       103          104              107          
%                       
