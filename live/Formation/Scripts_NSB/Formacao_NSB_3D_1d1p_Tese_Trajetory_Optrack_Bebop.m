...
% Ajusted Gains
% NSBF.pPar.K1 =   diag([0.8 0.8 0.8 2 4 6]);                  % kinematic control gain - controls amplitude
% NSBF.pPar.K2 =   1*diag([1 1 1 1 1 1]);                  % kinematic control gain - control saturation

% % Ajusted Gains Pouso
% NSBF.pPar.K1 =   diag([0.5 0.5 0.5 2 1 1.4]);                  % kinematic control gain - controls amplitude
% NSBF.pPar.K2 =   1*diag([1 1 1 1 1 1]);                  % kinematic control gain - control saturation

% Ajusted Gains Pouso
NSBF.pPar.K1 =   diag([0.5 0.5 0.5 2 2 4]);                  % kinematic control gain - controls amplitude
NSBF.pPar.K2 =   1*diag([1 1 1 1 1 1]);                  % kinematic control gain - control saturation
...
%         || abs(NSB.pPos.Qtil(6))>erroMax(6) % formation errors
    ...
    if toc(tc) > sampleTime                   
        ...
        % Compoe o set point de trajetoria        
        Xd = [rX*cos(w*t_atual);
              rY*sin(w*t_atual);
              0];
                           
        dXd = [-w*rX*sin(w*t_atual);
                w*rY*cos(w*t_atual);
                0];
                      

        % Atualiza posição para circulo
        NSBF.pPos.Qd(1:3) = Xd;
        NSBF.pPos.dQd(1:3) = dXd;
...
%         disp(NSBF.pPos.Qtil');
        ...
        if length(Rede.pMSG.getFrom)>2
            P.pSC.U  = Rede.pMSG.getFrom{2}(29:30);  % current velocities (robot )
            P1X       = Rede.pMSG.getFrom{2}(14+(1:12));   % current position (robot )
        end
