...
sampleTime = 1/30;
A.pPar.Ts = sampleTime;
Xod = [1 1 1 0]';                           % Posição incial Drone
A.pPos.X(1:4) = Xod;
...
P.rConnect;
Xop = [0 0 0 0]';                           % Posição inicial Pionner
P.rSetPose(Xop);
...
%       P = fNSBDynamicCompensator(P,pGains);
...
% NSBF.mFormationPerformace(index)
disp('----------------------------');
...
figure()
xAux = 1:1:length(NSBF.nFErr(2,:));
hold on
    plot(xAux, NSBF.nFErr(1,:));
    plot(xAux, NSBF.nFErr(2,:));
    title('Erro de Formação e Posição Normalizados');
    legend('Posição','Formação');
    grid on;
hold off
