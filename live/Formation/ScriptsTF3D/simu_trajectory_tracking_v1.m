...
F{2} = TriangularFormation3D;
F{3} = TriangularFormation3D;
...
%         disp('D3');
    ...
    if toc(t_Formation) > T_FORMATION
        ...
        A{1}.pPos.Xd(1:3) = Pd + [ 1.0 ; 0.0 ; 1 ]; % Posição desejada
        A{1}.pPos.Xd(7:9) = dPd;

        % A2
        A{2}.pPos.Xda = A{2}.pPos.Xd;   % save previous posture
        A{2}.pPos.Xd(1:3) = Pd + [ -1 ; 0 ; 1 ];
        A{2}.pPos.Xd(7:9) = dPd;        % Velocidade desejada
        
        % A3
        A{3}.pPos.Xda = A{2}.pPos.Xd;   % save previous posture
        A{3}.pPos.Xd(1:3) = Pd + [ 0 ; 1 ; 1 ];
        A{3}.pPos.Xd(7:9) = dPd;        % Velocidade desejada
