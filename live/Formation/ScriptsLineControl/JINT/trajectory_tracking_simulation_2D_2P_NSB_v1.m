...
% Formation 3D
LF{1} = LineFormationControl;   % P1 + D1 | POSIÇÃO
LF{2} = LineFormationControl;   % P1 + P2 | POSIÇÃO
LF{3} = LineFormationControl;   % P2 + D2 | POSIÇÃO
LF{4} = LineFormationControl;   % D1 + D2 | POSIÇÃO

for ii = 1:4
    LF{ii}.pPar.K1 = diag([  0.1   0.1   1.0   0.75    0.50   0.50  ]);     % kinematic control gain  - controls amplitude
    LF{ii}.pPar.K2 = diag([  0.5   0.5   0.5   0.50    0.25   0.25  ]);     % kinematic control gain - control saturation
end
...
%% Robots initial pose
% Pioneer P3DX
P{1}.rSetPose([ -0.5 0.0 0.0 0]');
P{1}.pSC.Ud = [0 ; 0];
P{1}.rSendControlSignals;    % Pioneer

P{2}.rSetPose([ 0 -0.5 0.01 0]');
P{2}.pSC.Ud = [0 ; 0];
P{2}.rSendControlSignals;    % Pioneer

% ArDrone
A{1}.pPos.X(1:3) = [ 0.0 0.5 0.7 ];
A{1}.pPos.X(6) = 0;

A{2}.pPos.X(1:3) = [ 0.5 -0.5 1.5 ];
A{2}.pPos.X(6) = 0;
...
LF{4}.pPos.X = [A{1}.pPos.X(1:3); A{2}.pPos.X(1:3)];
...
while toc(t)< T4
    ...
    if toc(t_Formation) > T_FORMATION
        ...
        LF{4}.pPos.X = [A{1}.pPos.X(1:3); A{2}.pPos.X(1:3)];    % A1 + A2 | POSIÇÃO
        ...
        LF{3}.pPos.Qd = Xd + [ 1.0 ; 0.0 ; 0.0 ;   1.5  ;  0.0  ; 0.0  ];
        LF{4}.pPos.Qd = Xd + [ 0.0 ; 0.0 ; 1.0 ;   sqrt(1.5) ;  pi/2-atan(0.5) ; 0.0  ];

        
        Estrategia = 'FFFF';
        for ii = 1:4
            LF{ii}.pPos.dQd = (LF{ii}.pPos.Qd - LF{ii}.pPos.Qda)/LF{ii}.pPar.Ts;
            LF{ii}.mFormationControlNSB(Estrategia(ii));             
            LF{ii}.mInvTrans;
        end
   
        % Atribuindo posições desejadas       
        % P1
        P{1}.pPos.Xd(1:3) = LF{1}.pPos.Xr(1:3); %(LF{1}.pPos.Xr(1:3)  + LF{2}.pPos.Xr(1:3))/2;         % Posição desejada
        P{1}.pPos.Xd(7:9) = LF{1}.pPos.dXr(1:3); %(LF{1}.pPos.dXr(1:3) + LF{2}.pPos.dXr(1:3))/2;        % Velocidade desejada
        
        % A1
        A{1}.pPos.Xda = A{1}.pPos.Xd;   % save previous posture
        A{1}.pPos.Xd(1:3) = LF{1}.pPos.Xr(4:6); %(LF{1}.pPos.Xr(4:6)  + LF{4}.pPos.Xr(1:3))/2; % Posição desejada
        A{1}.pPos.Xd(7:9) = LF{1}.pPos.dXr(4:6); %(LF{1}.pPos.dXr(4:6) + LF{4}.pPos.dXr(1:3))/2;
        
        % P2
        P{2}.pPos.Xd(1:3) = LF{2}.pPos.Xr(4:6); %(LF{2}.pPos.Xr(4:6)  + LF{3}.pPos.Xr(1:3))/2; % Posição desejada
        P{2}.pPos.Xd(7:9) = LF{2}.pPos.dXr(4:6); %(LF{2}.pPos.dXr(4:6) + LF{3}.pPos.dXr(1:3))/2;        % Velocidade desejada
        
        % A2
        A{2}.pPos.Xda = A{2}.pPos.Xd;   % save previous posture
        A{2}.pPos.Xd(1:3) = LF{4}.pPos.Xr(4:6); %(LF{3}.pPos.Xr(4:6)  + LF{4}.pPos.Xr(4:6))/2;
        A{2}.pPos.Xd(7:9) = LF{4}.pPos.dXr(4:6); %(LF{3}.pPos.dXr(4:6) + LF{4}.pPos.dXr(4:6))/2;        % Velocidade desejada
               
        disp(A{2}.pPos.Xd(1:3)')
