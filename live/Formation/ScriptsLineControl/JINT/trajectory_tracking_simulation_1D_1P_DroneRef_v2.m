...
LF.pPos.X = [A.pPos.X(1:3);P.pPos.X(1:3)];
LF.pPos.X = [1 ; 1 ; 3 ; 0 ; 0 ; 0];
...
%         P.pSC.Ud = [0; 0];
    ...
    if toc(t_Formation) > T_FORMATION
        ...
        LF.pPos.X = [A.pPos.X(1:3); P.pPos.X(1:3)];   % Posição dos membros da formação
        ...
        % Formation Control
        LF.pPar.K1 = diag([   20.0   20.0   0   1.0   2.0   2.0   ]);     % kinematic control gain  - controls amplitude
        LF.pPar.K2 = diag([    .1    .1     0    .1    .1    .1   ]);     % kinematic control gain - control saturation
...
%         LF.mFormationControl_NullSpace('P');                           
        ...
        % Pioneer
        P.pPos.Xd(1:3) = LF.pPos.Xr(4:6);             % Posição desejada
        P.pPos.Xd(7:9) = LF.pPos.dXr(4:6);            % Velocidade desejada

        % ArDrone
        A.pPos.Xda = A.pPos.Xd;    % save previous posture
        A.pPos.Xd(1:3) = LF.pPos.Xr(1:3);
        A.pPos.Xd(7:9) = LF.pPos.dXr(1:3);
