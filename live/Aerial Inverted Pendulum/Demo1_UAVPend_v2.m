...
while toc(t) < tmax
    if toc(tc) > ta
        ...
        % Sinal de controle
        eta = 0.1*diag([5 5 10 100 100 100 0.1 0.1])*(qd-q) + 0.1*diag([1 1 1 15 15 15 5 5])*(dqd-dq); % + ...
%         eta = 0.25*diag([5 5 5 20 20 10 10 10])*tanh(qd-q) + 0.35*diag([1 1 1 15 15 10 5 5])*tanh(dqd-dq); % + ...
