...
% Versão Original: Marcos Felipe e Wagner Casagrande
...
F = Formation_2Robots;   % classe formação

Rede = InfoSharing;

% Envia e lê dados iniciais para a rede
Rede.mSendMsg(P{1});
Rede.mSendMsg(P{2});
Rede.mReceiveMsg;
%%
...
while parar == 0
    ...
    % Salva Postura dos robos recebidos na ree
    if  ~isempty(Rede.pMSG.getFrom)
        F.pPos.tempX1 = Rede.pMSG.getFrom{1}(15:26);
        F.pPos.tempX2 = Rede.pMSG.getFrom{2}(15:26);
    else
        F.pPos.tempX1 = zeros(12,1);
        F.pPos.tempX2 = zeros(12,1);
    end
    ...
    if toc(t_controle)>0.1
        ...
        F.pPos.X = [ F.pPos.tempX1(1:6) ; F.pPos.tempX2(1:6)];
        F.mCalculaFormacao;
