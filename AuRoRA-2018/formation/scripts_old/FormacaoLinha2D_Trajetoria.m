...
Arq = fopen(['FL2dtrajElipseSim_' NomeArq '.txt'],'w');
cd(PastaAtual)
...
% LF.pPar.K1 = 1*diag([.45 .45 0.7 1.3]);    % ganho controlador cinemático
% LF.pPar.K2 = 1*diag([.28 .28 .5 .4]);      % ganho controlador cinemático

LF.pPar.K1 = 0.3*diag([.45 .45 0.7 1.3]);    % ganho controlador cinemático
LF.pPar.K2 = 0.3*diag([.28 .28 .5 .4]);      % ganho controlador cinemático
...
%         LF.pPos.Qd(4) = atan2(LF.pPos.dQd(2),LF.pPos.dQd(1));        
        ...
        if length(Rede.pMSG.getFrom) >= 1
            ...
            if  PA.pID > 1
                ...
                if ~isempty(Rede.pMSG.getFrom{1})
                    ...
                    PA.pPos.Xd(1:2) = LF.pPos.Xd(3:4);   % posições desejadas
                    PA.pPos.Xd(7:8) = LF.pPos.dXr(3:4);  % velocidades desejadas
