...
%                 sin(P.pPos.X(6)), +P.pPar.a*cos(P.pPos.X(6))];
          angulo_trailer_mundo = P.pPos.X(6)-(pi/2) + P.pPot1.Data*pi/180;
          K = [ -sin(angulo_trailer_mundo), 0.19*cos(angulo_trailer_mundo); ...
                cos(angulo_trailer_mundo), 0.19*sin(angulo_trailer_mundo)];
            
%            K = [ -sin(P.pPos.X(6)-(pi/2)), -P.pPar.a*cos(P.pPos.X(6)-(pi/2)); ...
%                 cos(P.pPos.X(6)-(pi/2)), -P.pPar.a*sin(P.pPos.X(6)-(pi/2))];
            
%           P.pPos.Xtil = P.pPos.Xd - P.pPos.X;

          %%% -- Pose Trailer
          
          x_t = P.pPos.Xc(1) + L0*sin(P.pPos.X(6)-(pi/2)) + (L1 + 0.19)*sin(P.pPos.X(6)-(pi/2) + P.pPot1.Data*pi/180);
          y_t = P.pPos.Xc(2) + L0*cos(P.pPos.X(6)-(pi/2)) - (L1 + 0.19)*cos(P.pPos.X(6)-(pi/2) + P.pPot1.Data*pi/180);
          pose = [x_t; y_t];
          centro_robo = [P.pPos.Xc(1); P.pPos.Xc(2)];
          
          dist = norm(P.pPos.Xd(1:2) - pose);
...
%           t0_ct_hist = [t0_ct_hist t0_ct];
          pose_trailer = [pose_trailer pose];
...
% hold on;
% grid on;
% plot(t_hist,vel_hist(1,:));
% plot(t_hist,t1_hist(1,:));
% plot(t_hist,t0_c_hist(1,:));
% plot(t_hist,t1_c_hist(1,:));
% legend('Velocidade Linear - trator','Velocidade Linear - trailer 1','Velocidade Linear trator - inverso', 'Velocidade linear - centro trator');
% xlabel('t');
...
% hold on;
% grid on;
% plot(t_hist,vel_hist(2,:));
% plot(t_hist,t1_hist(2,:));
% plot(t_hist,t0_c_hist(2,:));
% plot(t_hist,t0_ct_hist(2,:));
% legend('Velocidade Angular - trator','Velocidade Angular - trailer 1', 'Velocidade Angular trator - inverso', 'Velocidade angular - centro trator');
% xlabel('t');
...
grid on;
plot(xd_hist(1,:),xd_hist(2,:));
plot(pose_trailer(1,:),pose_trailer(2,:)); 
