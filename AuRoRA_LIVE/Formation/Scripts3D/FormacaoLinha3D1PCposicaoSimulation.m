...
% Ganhos Valentim
% LF.pPar.K1 = 1*diag([1.0 1.0 1.0 0.8 0.08 0.08]);    % kinematic control gain  - controls amplitude
% LF.pPar.K2 = 1*diag([0.1 0.1 0.025 0.1 0.01 0.01]);        % kinematic control gain - control saturation
...
time = 30;    % time to change desired positions [s]
...
while toc(t)< size(Qd,1)*(time)  %abs(LF.pPos.Qtil(1))>erroMax(1) || abs(LF.pPos.Qtil(2))>erroMax(2) || ...
...
%         || abs(LF.pPos.Qtil(6))>erroMax(6) % formation errors
    ...
    if toc(tc) > 1/30
        ...
        LF.mInvTrans;
        % Pioneer
        P.pPos.Xd(1:2) = LF.pPos.Xr(1:2);        % desired position
        P.pPos.Xd(7:8) = LF.pPos.dXr(1:2);        % desired position
