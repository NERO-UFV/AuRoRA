...
% Robot
P = Pioneer3DX(6);   % the ID must be diferent of the pioneer in the server machine to avoid conflict on NetDataShare
...
X = Rede.pMSG.getFrom{3}(14+(1:12));  % get position from network (optitrack)
...
while true % toc(t) < tsim
    ...
    if toc(tc) > 0.030
        ...
        if ~isempty(Rede.pMSG.getFrom{3})
            ...
            P.pPos.Xd = Rede.pMSG.getFrom{3}(2+(1:12));    % desired [for plot purposes only]
            P.pSC.Ud  = Rede.pMSG.getFrom{3}(27:28);       % control signal
            X         = Rede.pMSG.getFrom{3}(14+(1:12));   % traveled (optitrack) [to correct odometry]
