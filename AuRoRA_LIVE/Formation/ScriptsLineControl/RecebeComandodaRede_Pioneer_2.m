...
% Robot
P = Pioneer3DX(5);   % the ID must be diferent of the pioneer in the server machine to avoid conflict on NetDataShare
...
X = Rede.pMSG.getFrom{2}(14+(1:12));  % get position from network (optitrack)
...
while true % toc(t) < tsim
    ...
    if toc(tc) > 0.030
        ...
        if ~isempty(Rede.pMSG.getFrom{2})
            ...
            P.pPos.Xd = Rede.pMSG.getFrom{2}(2+(1:12));    % desired [for plot purposes only]
            P.pSC.Ud  = Rede.pMSG.getFrom{2}(27:28);       % control signal
            X         = Rede.pMSG.getFrom{2}(14+(1:12));   % traveled (optitrack) [to correct odometry]
