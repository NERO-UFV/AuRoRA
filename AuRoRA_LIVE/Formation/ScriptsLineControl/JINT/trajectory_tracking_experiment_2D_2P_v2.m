...
LF{2}.pPar.K1 = diag([  1.0   1.0   0.0   20.0    20.0   20.0  ]);     % kinematic control gain  - controls amplitude
LF{2}.pPar.K2 = diag([  0.5   0.5   0.0   0.1    0.1   0.1    ]);   % kinematic control gain - control saturation

for ii = [1 3]
    LF{ii}.pPar.K1 = diag([  1.0   1.0   0.0   3.0    3.0   3.0  ]);     % kinematic control gain  - controls amplitude
...
A{1}.pPos.Xd(1:3) = [ 0.0 ; 0.0 ; 1.50001 ]; % Posição desejada
A{1}.pPos.Xd(7:9) = [0;0;0];

% A2
A{2}.pPos.Xda = A{2}.pPos.Xd;   % save previous posture
A{2}.pPos.Xd(1:3) = [ -1.0 ; -1.0 ; 1.5 ];
A{2}.pPos.Xd(7:9) = [0;0;0];        % Velocidade desejada
...
%         if toc(t_control) > T_CONTROL
        ...
        if toc(t_ArDrone_1) >  A{1}.pPar.Ts
            t_ArDrone_1 = tic;
    %         disp('D1');
            ...
            A{1} = cInverseDynamicController(A{1});
            A{1} = J.mControl(A{1});
            A{1}.rSendControlSignals;               % Enviar sinal de controle para o robô
        end

        if toc(t_ArDrone_2) >  A{1}.pPar.Ts
            t_ArDrone_2 = tic;
    %         disp('D2');
            ...
            A{2} = cInverseDynamicController(A{2});
            A{2} = J.mControl(A{2}); 
...
% A1
A{1}.pPos.Xda = A{1}.pPos.Xd;   % save previous posture
A{1}.pPos.Xd(1:3) = Xd(1:3) + [ 0.0 ; 0.0 ; 1.50001 ]; % Posição desejada
A{1}.pPos.Xd(7:9) = dXd(1:3);

% P2
P{2}.pPos.Xd(1:3) = Xd(1:3) + [ -1.0 ; -1.0 ; 0.000001 ]; % Posição desejada
...
T_PLOT = 50;       % Período de plotagem em tempo real
...
T3 =  5.1 + T2;        % Andando com o Drone pousado
...
%             P{1} = fDynamicController(P{1},Pgains);    % Controlador
...
%             Pgains = [ 0.10  0.10  0.8  0.8  0.75  0.75  0.10  0.03 ];
%             P{2} = fDynamicController(P{2},Pgains);    % Pioneer Dynamic Compensator
    ...
    %         disp('D2');
        ...
        if toc(t_Formation) > T_FORMATION
            ...
            LF{1}.pPos.Qd = Xd + [ 0.0 ; 0.0 ; 0.0 ;   1.5   ;  0.0  ; 0.0  ];        
            LF{2}.pPos.Qd = Xd + [ 0.0 ; 0.0 ; 0.0 ;   1.0   ;  pi/4 ; pi/4  ];
            LF{3}.pPos.Qd = Xd + [ -1.0 ; -1.0 ; 0.0 ;   1.5  ;  0.0  ; 0.0  ];
            LF{4}.pPos.Qd = Xd + [ -1.0 ; -1.0 ; 1.5 ;   1.5  ;  -pi/4 ; -pi/4 ];
    ...
    %             LF{ii}.mFormationControl;
            ...
            % A1
            A{1}.pPos.Xda = A{1}.pPos.Xd;   % save previous posture
            A{1}.pPos.Xd(1:3) = LF{4}.pPos.Xr(4:6); % Posição desejada
            A{1}.pPos.Xd(7:9) = LF{4}.pPos.dXr(4:6);
...
%             A{1}.pPos.Xd(12) = P{1}.pPos.X(12);  % dPsi
            ...
            % A2
            A{2}.pPos.Xda = A{2}.pPos.Xd;   % save previous posture
            A{2}.pPos.Xd(1:3) = Xd + [ -1.0 ; -1.0 ; 1.5 ];%LF{3}.pPos.Xr(4:6); %(LF{3}.pPos.Xr(4:6)  + LF{4}.pPos.Xr(4:6))/2;
            A{2}.pPos.Xd(7:9) = dXd; %LF{3}.pPos.dXr(4:6); %(LF{3}.pPos.dXr(4:6) + LF{4}.pPos.dXr(4:6))/2;        % Velocidade desejada
%             A{2}.pPos.Xd(6) = P{2}.pPos.X(6);
%             A{2}.pPos.Xd(12) = P{2}.pPos.X(12);  % dPsi
            ...
            if toc(t) > T2 && toc(t) < T3
                ...
                if toc(t_ArDrone_1) > T_ARDRONE
                    LF{3}.pPos.Qd = Xd + [ -1.0 ; -1.0 ; 0.0 ;   0.5  ;  0.0  ; 0.0  ];
                    LF{4}.pPos.Qd = Xd + [ -1.0 ; -1.0 ; 0.5 ;   1.0  ;  -pi/4 ; -pi/4 ];
                end    
            end
