...
F{1}.pPos.Xr = [ [.75  0  1.5]' ; [-.75  0  1.5]' ; [-.75  0  1.5]' ];
...
%             B{2}.pSC.Ud = [  0   0   0   0   0   0  ]';
        ...
        if toc(t_Bebop_3) > T_BEBOP
            ...
            B{3}.cInverseDynamicController_Compensador;  % Controlador 
...
%             else 
                ...
                Oda = Od;
                Od = [   pi      0      (-pi -0.3218 + ((-pi + 0.3218)*toc(t))/(Tf)) ]';
                if abs(Od(3)) > pi
                    if Od(3) > 0
                        Od(3) = Od(3) - 2*pi;
                    else
...
%             triangle_d = plot3([F{1}.pPos.Xr(1) B{2}.pPos.Xd(1) B{3}.pPos.Xd(1) F{1}.pPos.Xr(1)],...
