classdef JoyControl < handle
    ...
    properties
        ...
        pDigital   % Button
        pAnalog    % Axis
        pFlag      % flag for multiple purposes
        pSwitch    % flag that allows the exchange of robots
        pIsConnected
        pSNES  % flag to support SNES gamepad
    ...
    methods
        function obj = JoyControl(ID)
            ...
            obj.pID = ID;
            obj.pIsConnected = 0;
            obj.pFlag = 0;   % for instance, use it to exit while  
            obj.pSwitch = 0;
            obj.pSNES = 0;
            mConnect(obj);
        end
        ...
        function robot = mControl(obj,robot)
            ...
            if obj.pIsConnected == 1
                ...
                elseif type(2) == 'P' || type(2) == 'p'  % Para a classe RPioneer
                    if sum(abs(obj.pAnalog)) > 0
                        % Joystick Control signals
                        temp  = obj.pAnalog'.*[1 -1 -1 -1 1]';
                        robot.pSC.Ud = [0.75*temp(2); 0.5*temp(4)];
                    ...
                    while obj.pDigital(5)==1
                        obj.pSwitch = -1;
                        mRead(obj)
                    end
                    while obj.pDigital(6)==1
                        obj.pSwitch = 1;
                        mRead(obj)
                    end
                    % ====================================================
