...
%         P.pPos.Xd(8) = r*w/tmax*abs((cos(w*toc(t)/tmax)^(1/2)))*sign(cos(w*toc(t)/tmax));
        ...
        if toc(t)<40
            P.pPos.Xd(1) = 2.2462;
            P.pPos.Xd(2) = P.pPos.X(2) + 0.8;
            if P.pPos.Xd(2) > 3.5
                P.pPos.Xd(2) = 3.5;
            end
        elseif toc(t)>40 && toc(t)<60
            P.pPos.Xd(1) = P.pPos.X(1) - 0.8;
            if P.pPos.X(1)<-2.1
                P.pPos.X(1) = -2.1;
            end
            P.pPos.Xd(2) = 3.5;
...
%%
figure(3)
% subplot(121stem(abs(D(:)),'k.','MarkerSize',15)
stem(abs(D(:)),'Marker','.','Color', [0,0.447,0.741],'MarkerSize',15,'LineWidth',1.2)
hold on
stem(idx,abs(D(idx)),'r.','MarkerSize',15,'LineWidth',1.2)
plot((sum(abs(D))/181).*ones(1,182),'r--','LineWidth',1)
xlim([0,181])
legend('|d_n| \in \bf{U}','\bf{O}','d_{th}')
grid on
hold off
% subplot(122)
