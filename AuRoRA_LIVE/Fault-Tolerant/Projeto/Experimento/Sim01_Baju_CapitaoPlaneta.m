...
%     P{2} = RPioneer(1,'P2'); % Pìoneer3DX Experimento
%     P{3} = RPioneer(1,'P3'); % Pìoneer3DX Experimento
    P{1} = Pioneer3DX(1); % Pìoneer3DX Simulação
    P{2} = Pioneer3DX(2); % Pìoneer3DX Simulação
    P{3} = Pioneer3DX(3); % Pìoneer3DX Simulação
...
TF.pPar.K1 = .8*diag([.6  .6  1   1.5   2.2 2.2 2.2 2.2 1.5]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    Qd{1}(:,2) = [Xfi; Yfi; Zfi;
                  -BETAfi/2+BETAfi/2*(toc(T)-T_MAX/4)/(T_MAX/4); PHIfi; PSIfi;
                  Pfi; Qfi; BETAfi];

    % Missão 2 - Drone 1 afasta para Drone 2 levantar voo
    Qd{2}(:,1) = [Xfi; Yfi; Zfi;
                  -BETAfi*toc(T)/(T_MAX/4); PHIfi; PSIfi;
                  Pfi; Qfi; BETAfi];
              
    % Missão 3 - Drone 2 levantar voo
    Qd{3}(:,1) = [Xfi; Yfi; Zfi;
                  -BETAfi; PHIfi; PSIfi;
                  Pfi; 0.8+(Qfi-0.8)*toc(T)/(T_MAX/4); BETAfi];
              
    % Missão 4 - Formação atua para deixar o Drone 1 em cima do Pioneer
    Qd{4}(:,1) = [Xfi; Yfi; Zfi;
                  -BETAfi+BETAfi*toc(T)/(T_MAX/4); PHIfi; PSIfi;
                  Pfi; Qfi; BETAfi];
              
    % Missão 5 - Formação atua para deixar o Drone 1 em cima do Pioneer
    Qd{5}(:,1) = [Xfi; Yfi; Zfi;
                  THETAfi; PHIfi; PSIfi;
                  Pfi+(H_LAND-Pfi)*toc(T)/(T_MAX/4); Qfi; BETAfi];
              
    % Missão 6 - Drone 2 retorna para posição acima do Pioneer
    Qd{6}(:,1) = [Xfi; Yfi; Zfi;
                  -BETAfi*toc(T)/(T_MAX/4); PHIfi; PSIfi;
                  Pfi; Qfi; BETAfi];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    ...
    switch MISSAO(ETAPA,1)
        case 1    
            if toc(T) > T_MAX/4 && MISSAO(ETAPA,2) == 1
                ETAPA = ETAPA + 1;
                disp(MISSAO(ETAPA,:))
            end
    end
